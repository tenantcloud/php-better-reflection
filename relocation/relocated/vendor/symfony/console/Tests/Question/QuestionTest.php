...
class QuestionTest extends \TenantCloud\BetterReflection\Relocated\PHPUnit\Framework\TestCase
{
    private $question;
    protected function setUp() : void
    {
        parent::setUp();
        $this->question = new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Question\Question('Test question');
    }
    ...
    {
        $question = new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Question\Question('Test question', 'Default value');
    ...
    {
        $this->question->setAutocompleterCallback(function (string $input) : array {
            return [];
        });
        $this->expectException(\LogicException::class);
        $this->expectExceptionMessage('A hidden question cannot use the autocompleter.');
        $this->question->setHidden(\true);
    }
    ...
    {
        return ['array' => [['a', 'b', 'c', 'd'], ['a', 'b', 'c', 'd']], 'associative array' => [['a' => 'c', 'b' => 'd'], ['a', 'b', 'c', 'd']], 'iterator' => [new \ArrayIterator(['a', 'b', 'c', 'd']), ['a', 'b', 'c', 'd']], 'null' => [null, null]];
    }
    /**
    ...
    public function testGetSetAutocompleterValues($values, $expectValues)
    {
        ...
        self::assertSame($expectValues, $this->question->getAutocompleterValues());
    }
    public function providerSetAutocompleterValuesInvalid()
    {
        return [['Potato'], [new \stdClass()], [\false]];
    }
    /**
    ...
    {
        self::expectException(\TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Exception\InvalidArgumentException::class);
        self::expectExceptionMessage('Autocompleter values can be either an array, "null" or a "Traversable" object.');
        $this->question->setAutocompleterValues($values);
    }
    public function testSetAutocompleterValuesWithTraversable()
    {
        $question1 = new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Question\Question('Test question 1');
        $iterator1 = $this->getMockForAbstractClass(\IteratorAggregate::class);
        $iterator1->expects($this->once())->method('getIterator')->willReturn(new \ArrayIterator(['Potato']));
        $question1->setAutocompleterValues($iterator1);
        $question2 = new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Question\Question('Test question 2');
        $iterator2 = $this->getMockForAbstractClass(\IteratorAggregate::class);
        $iterator2->expects($this->once())->method('getIterator')->willReturn(new \ArrayIterator(['Carrot']));
