 ...
 */
class ProgressBarTest extends \TenantCloud\BetterReflection\Relocated\PHPUnit\Framework\TestCase
{
    private $colSize;
    ...
    {
        $bar = new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Helper\ProgressBar($output = $this->getOutputStream(), 0, 0);
        $bar->start();
        $bar->advance();
        $bar->start();
    ...
    {
        $bar = new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Helper\ProgressBar($output = $this->getOutputStream(), 10, 0);
        $bar->setProgress(9);
        $bar->advance();
        $bar->advance();
        \rewind($output->getStream());
    ...
    {
        $expected = '  0/10 [>---------------------------]   0%' . $this->generateOutput(' 10/10 [============================] 100%');
    ...
    {
        $bar = new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Helper\ProgressBar($output = $this->getOutputStream(), 50, 0);
        $bar->display();
        \rewind($output->getStream());
    ...
    {
        $bar = new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Helper\ProgressBar($output = $this->getOutputStream(\true, \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Output\StreamOutput::VERBOSITY_QUIET), 50, 0);
        $bar->display();
        \rewind($output->getStream());
    ...
    public function testPercent()
    {
        ...
        $bar->advance();
        \rewind($output->getStream());
        $this->assertEquals('  0/50 [>---------------------------]   0%' . $this->generateOutput('  1/50 [>---------------------------]   2%') . $this->generateOutput('  2/50 [=>--------------------------]   4%'), \stream_get_contents($output->getStream()));
    }
    public function testOverwriteWithShorterLine()
    {
        ...
        $bar->advance();
        \rewind($output->getStream());
        $this->assertEquals('  0/50 [>---------------------------]   0%' . $this->generateOutput('  1/50 [>---------------------------]   2%') . $this->generateOutput('  2/50 [=>--------------------------]'), \stream_get_contents($output->getStream()));
    }
    public function testOverwriteWithSectionOutput()
    {
        ...
        $bar = new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Helper\ProgressBar($output, 50, 0);
        $bar->start();
        $bar->display();
        $bar->advance();
        $bar->advance();
        \rewind($output->getStream());
        $this->assertEquals('  0/50 [>---------------------------]   0%' . \PHP_EOL . "\33[1A\33[0J" . '  1/50 [>---------------------------]   2%' . \PHP_EOL . "\33[1A\33[0J" . '  2/50 [=>--------------------------]   4%' . \PHP_EOL, \stream_get_contents($output->getStream()));
    }
    public function testOverwriteMultipleProgressBarsWithSectionOutputs()
    {
        $sections = [];
        $stream = $this->getOutputStream(\true);
        $output1 = new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Output\ConsoleSectionOutput($stream->getStream(), $sections, $stream->getVerbosity(), $stream->isDecorated(), new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Formatter\OutputFormatter());
        $output2 = new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Output\ConsoleSectionOutput($stream->getStream(), $sections, $stream->getVerbosity(), $stream->isDecorated(), new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Formatter\OutputFormatter());
        $progress = new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Helper\ProgressBar($output1, 50, 0);
        $progress2 = new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Helper\ProgressBar($output2, 50, 0);
        $progress->start();
        $progress2->start();
        $progress2->advance();
        $progress->advance();
    ...
    public function testSetCurrentProgress()
    {
        ...
        $bar->setProgress(25);
        \rewind($output->getStream());
        $this->assertEquals('  0/50 [>---------------------------]   0%' . $this->generateOutput('  1/50 [>---------------------------]   2%') . $this->generateOutput(' 15/50 [========>-------------------]  30%') . $this->generateOutput(' 25/50 [==============>-------------]  50%'), \stream_get_contents($output->getStream()));
    }
    public function testSetCurrentBeforeStarting()
    {
        $bar = new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Helper\ProgressBar($this->getOutputStream(), 0, 0);
        $bar->setProgress(15);
        $this->assertNotNull($bar->getStartTime());
    }
    public function testRedrawFrequency()
    {
        $bar = new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Helper\ProgressBar($output = $this->getOutputStream(), 6, 0);
    ...
    {
        $bar = new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Helper\ProgressBar($output = $this->getOutputStream(), 200, 0);
        $bar->start();
        $bar->display();
        $bar->advance(199);
        $bar->advance();
        \rewind($output->getStream());
        $this->assertEquals('   0/200 [>---------------------------]   0%' . $this->generateOutput(' 199/200 [===========================>]  99%') . $this->generateOutput(' 200/200 [============================] 100%'), \stream_get_contents($output->getStream()));
    }
    public function testNonDecoratedOutput()
    {
        $bar = new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Helper\ProgressBar($output = $this->getOutputStream(\false), 200, 0);
        $bar->start();
        for ($i = 0; $i < 200; ++$i) {
            $bar->advance();
        }
    ...
    {
        $bar = new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Helper\ProgressBar($output = $this->getOutputStream(\false), 50, 0);
        $bar->start();
        $bar->setProgress(25);
        $bar->clear();
        $bar->setProgress(50);
    ...
    {
        $bar = new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Helper\ProgressBar($output = $this->getOutputStream(\false), 0, 0);
        $bar->start();
        $bar->advance();
        \rewind($output->getStream());
    ...
    public function testParallelBars()
    {
        ...
        $bar1 = new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Helper\ProgressBar($output, 2, 0);
        $bar2 = new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Helper\ProgressBar($output, 3, 0);
        $bar2->setProgressCharacter('#');
        $bar3 = new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Helper\ProgressBar($output, 0, 0);
        $bar1->start();
        $output->write("\n");
        $bar2->start();
        $output->write("\n");
    ...
    {
        $output = $this->getOutputStream();
        $bar = new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Helper\ProgressBar($output, 0, 0);
        $bar->start();
        $bar->advance();
        $bar->advance();
        $bar->advance();
    ...
    {
        \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Helper\ProgressBar::setPlaceholderFormatterDefinition('remaining_steps', function (\TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Helper\ProgressBar $bar) {
            return $bar->getMaxSteps() - $bar->getProgress();
        });
        $bar = new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Helper\ProgressBar($output = $this->getOutputStream(), 3, 0);
    ...
    {
        \putenv('COLUMNS=156');
        $bar = new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Helper\ProgressBar($output = $this->getOutputStream(), 15, 0);
        \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Helper\ProgressBar::setPlaceholderFormatterDefinition('memory', function (\TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Helper\ProgressBar $bar) {
            static $i = 0;
            $mem = 100000 * $i;
            $colors = $i++ ? '41;37' : '44;37';
            return "\33[" . $colors . 'm ' . \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Helper\Helper::formatMemory($mem) . " \33[0m";
        });
    ...
    public function testIterate()
    {
        ...
        \rewind($output->getStream());
        $this->assertEquals(' 0/2 [>---------------------------]   0%' . $this->generateOutput(' 1/2 [==============>-------------]  50%') . $this->generateOutput(' 2/2 [============================] 100%'), \stream_get_contents($output->getStream()));
    }
    ...
    {
        $bar = new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Helper\ProgressBar($output = $this->getOutputStream());
    ...
    {
        $bar = new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Helper\ProgressBar($output = $this->getOutputStream(), 0, 0);
        $bar->setRedrawFrequency(4);
        // disable step based redraws
        $bar->start();
        $bar->setProgress(1);
        // No treshold hit, no redraw
        $bar->maxSecondsBetweenRedraws(2);
        \sleep(1);
        $bar->setProgress(2);
        // Still no redraw because it takes 2 seconds for a redraw
        \sleep(1);
        $bar->setProgress(3);
        // 1+1 = 2 -> redraw finally
        $bar->setProgress(4);
        // step based redraw freq hit, redraw even without sleep
        $bar->setProgress(5);
        // No treshold hit, no redraw
        $bar->maxSecondsBetweenRedraws(3);
        \sleep(2);
        $bar->setProgress(6);
        // No redraw even though 2 seconds passed. Throttling has priority
        $bar->maxSecondsBetweenRedraws(2);
        $bar->setProgress(7);
        // Throttling relaxed, draw
        \rewind($output->getStream());
    ...
    public function testMinSecondsBetweenRedraws()
    {
        ...
        $bar->start();
        $bar->setProgress(1);
        // Too fast, should not draw
        \sleep(1);
        $bar->setProgress(2);
        // 1 second passed, draw
        $bar->minSecondsBetweenRedraws(2);
        \sleep(1);
        $bar->setProgress(3);
        // 1 second passed but we changed threshold, should not draw
        \sleep(1);
        $bar->setProgress(4);
        // 1+1 seconds = 2 seconds passed which conforms threshold, draw
        $bar->setProgress(5);
        // No treshold hit, no redraw
        \rewind($output->getStream());
    ...
    {
        $bar = new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Helper\ProgressBar($output = $this->getOutputStream(), 2);
        $bar->start();
        $bar->advance();
        $bar->display();
        \rewind($output->getStream());
        $this->assertEquals(' 0/2 [>---------------------------]   0%' . $this->generateOutput(' 1/2 [==============>-------------]  50%'), \stream_get_contents($output->getStream()));
    }
}
