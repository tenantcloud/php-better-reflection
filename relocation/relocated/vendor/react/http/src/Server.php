...
namespace TenantCloud\BetterReflection\Relocated\React\Http;
...
use TenantCloud\BetterReflection\Relocated\React\Http\Io\MiddlewareRunner;
use TenantCloud\BetterReflection\Relocated\React\Http\Io\StreamingServer;
use TenantCloud\BetterReflection\Relocated\React\Http\Middleware\LimitConcurrentRequestsMiddleware;
use TenantCloud\BetterReflection\Relocated\React\Http\Middleware\StreamingRequestMiddleware;
use TenantCloud\BetterReflection\Relocated\React\Http\Middleware\RequestBodyBufferMiddleware;
use TenantCloud\BetterReflection\Relocated\React\Http\Middleware\RequestBodyParserMiddleware;
 ...
 */
final class Server extends \TenantCloud\BetterReflection\Relocated\Evenement\EventEmitter
{
    /**
    ...
    public function __construct(\TenantCloud\BetterReflection\Relocated\React\EventLoop\LoopInterface $loop)
    {
        ...
        foreach ((array) $requestHandlers as $handler) {
            if ($handler instanceof \TenantCloud\BetterReflection\Relocated\React\Http\Middleware\StreamingRequestMiddleware) {
                $streaming = \true;
                break;
            }
        }
        ...
        if (!$streaming) {
            ...
            if ($concurrency !== null) {
                $middleware[] = new \TenantCloud\BetterReflection\Relocated\React\Http\Middleware\LimitConcurrentRequestsMiddleware($concurrency);
            }
            $middleware[] = new \TenantCloud\BetterReflection\Relocated\React\Http\Middleware\RequestBodyBufferMiddleware($maxSize);
            ...
            if ($enablePostDataReading !== '') {
                $middleware[] = new \TenantCloud\BetterReflection\Relocated\React\Http\Middleware\RequestBodyParserMiddleware();
            }
        }
        $middleware = \array_merge($middleware, $requestHandlers);
        $this->streamingServer = new \TenantCloud\BetterReflection\Relocated\React\Http\Io\StreamingServer($loop, new \TenantCloud\BetterReflection\Relocated\React\Http\Io\MiddlewareRunner($middleware));
        $that = $this;
    ...
    private function getConcurrentRequestsLimit($memory_limit, $post_max_size)
    {
        ...
        $availableMemory = \TenantCloud\BetterReflection\Relocated\React\Http\Io\IniUtil::iniSizeToBytes($memory_limit) / 2;
        $concurrentRequests = (int) \ceil($availableMemory / \TenantCloud\BetterReflection\Relocated\React\Http\Io\IniUtil::iniSizeToBytes($post_max_size));
    ...
    {
        $maxSize = \TenantCloud\BetterReflection\Relocated\React\Http\Io\IniUtil::iniSizeToBytes($post_max_size === null ? \ini_get('post_max_size') : $post_max_size);
