...
use TenantCloud\BetterReflection\Relocated\RingCentral\Psr7 as gPsr;
/**
 * @event response
 * @event drain
 * @event error
 ...
 * @internal
 */
class Request extends \TenantCloud\BetterReflection\Relocated\Evenement\EventEmitter implements \TenantCloud\BetterReflection\Relocated\React\Stream\WritableStreamInterface
{
    const STATE_INIT = 0;
    ...
    public function __construct(\TenantCloud\BetterReflection\Relocated\React\Socket\ConnectorInterface $connector, \TenantCloud\BetterReflection\Relocated\React\Http\Client\RequestData $requestData)
    {
    ...
    private function writeHead()
    {
        ...
        $promise->then(function (\TenantCloud\BetterReflection\Relocated\React\Socket\ConnectionInterface $stream) use($requestData, &$streamRef, &$stateRef, &$pendingWrites, $that) {
            $streamRef = $stream;
            ...
            $stateRef = \TenantCloud\BetterReflection\Relocated\React\Http\Client\Request::STATE_HEAD_WRITTEN;
    ...
    public function handleData($data)
    {
        ...
        if (\false !== \strpos($this->buffer, "\r\n\r\n") || \false !== \strpos($this->buffer, "\n\n")) {
            try {
                $response = \TenantCloud\BetterReflection\Relocated\RingCentral\Psr7\parse_response($this->buffer);
    ...
    protected function connect()
    {
        ...
        if ($scheme !== 'https' && $scheme !== 'http') {
            return \TenantCloud\BetterReflection\Relocated\React\Promise\reject(new \InvalidArgumentException('Invalid request URL given'));
        }
