...
use TenantCloud\BetterReflection\Relocated\React\Http\Io\ReadableBodyStream;
use TenantCloud\BetterReflection\Relocated\React\Http\Io\Sender;
use TenantCloud\BetterReflection\Relocated\React\Http\Io\Transaction;
...
class Browser
{
    ...
    {
        $this->transaction = new \TenantCloud\BetterReflection\Relocated\React\Http\Io\Transaction(\TenantCloud\BetterReflection\Relocated\React\Http\Io\Sender::createFromLoop($loop, $connector), $loop);
    }
    /**
    ...
    public function withBase($baseUrl)
    {
        ...
        $browser->baseUrl = new \TenantCloud\BetterReflection\Relocated\RingCentral\Psr7\Uri($baseUrl);
    ...
    private function requestMayBeStreaming($method, $url, array $headers = array(), $body = '')
    {
        if ($this->baseUrl !== null) {
            ...
            $url = \TenantCloud\BetterReflection\Relocated\RingCentral\Psr7\Uri::resolve($this->baseUrl, $url);
        }
        if ($body instanceof \TenantCloud\BetterReflection\Relocated\React\Stream\ReadableStreamInterface) {
            $body = new \TenantCloud\BetterReflection\Relocated\React\Http\Io\ReadableBodyStream($body);
        }
        return $this->transaction->send(new \TenantCloud\BetterReflection\Relocated\RingCentral\Psr7\Request($method, $url, $headers, $body, $this->protocolVersion));
    }
}
