...
use TenantCloud\BetterReflection\Relocated\React\Http\Io\HttpBodyStream;
use TenantCloud\BetterReflection\Relocated\React\Http\Io\PauseBufferStream;
...
final class LimitConcurrentRequestsMiddleware
{
    ...
    public function __invoke(\TenantCloud\BetterReflection\Relocated\Psr\Http\Message\ServerRequestInterface $request, $next)
    {
        ...
        if ($this->pending < $this->limit) {
            ...
            return $this->await(\TenantCloud\BetterReflection\Relocated\React\Promise\resolve($response));
        }
        ...
        if ($body instanceof \TenantCloud\BetterReflection\Relocated\React\Stream\ReadableStreamInterface) {
            ...
            $size = $body->getSize();
            $body = new \TenantCloud\BetterReflection\Relocated\React\Http\Io\PauseBufferStream($body);
            $body->pauseImplicit();
            // replace with buffering body to ensure any readable events will be buffered
            $request = $request->withBody(new \TenantCloud\BetterReflection\Relocated\React\Http\Io\HttpBodyStream($body, $size));
        }
        ...
        \end($queue);
        $id = \key($queue);
        $deferred = new \TenantCloud\BetterReflection\Relocated\React\Promise\Deferred(function ($_, $reject) use(&$queue, $id) {
        ...
        return $deferred->promise()->then(function () use($request, $next, $body, &$pending, $that) {
            ...
            if ($body instanceof \TenantCloud\BetterReflection\Relocated\React\Http\Io\PauseBufferStream) {
                $body->resumeImplicit();
            }
            // if the next handler returns a pending promise, we have to
            // await its resolution before invoking next queued request
            return $that->await(\TenantCloud\BetterReflection\Relocated\React\Promise\resolve($response));
        });
    }
    /**
     * @internal
     ...
     */
    public function await(\TenantCloud\BetterReflection\Relocated\React\Promise\PromiseInterface $promise)
    {
        $that = $this;
        return $promise->then(function ($response) use($that) {
            ...
            $that->processQueue();
            return \TenantCloud\BetterReflection\Relocated\React\Promise\reject($error);
        });
    }
    /**
     * @internal
