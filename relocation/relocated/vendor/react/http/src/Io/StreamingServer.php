...
use TenantCloud\BetterReflection\Relocated\React\Http\Message\Response;
use TenantCloud\BetterReflection\Relocated\React\Http\Message\ServerRequest;
...
use TenantCloud\BetterReflection\Relocated\React\Socket\ServerInterface;
 ...
 * @internal
 */
final class StreamingServer extends \TenantCloud\BetterReflection\Relocated\Evenement\EventEmitter
{
    private $callback;
    private $parser;
    private $loop;
     ...
     * @see self::listen()
     */
    public function __construct(\TenantCloud\BetterReflection\Relocated\React\EventLoop\LoopInterface $loop, $requestHandler)
    {
        ...
        $this->parser = new \TenantCloud\BetterReflection\Relocated\React\Http\Io\RequestHeaderParser();
        $that = $this;
        $this->parser->on('headers', function (\TenantCloud\BetterReflection\Relocated\Psr\Http\Message\ServerRequestInterface $request, \TenantCloud\BetterReflection\Relocated\React\Socket\ConnectionInterface $conn) use($that) {
            $that->handleRequest($conn, $request);
        });
        $this->parser->on('error', function (\Exception $e, \TenantCloud\BetterReflection\Relocated\React\Socket\ConnectionInterface $conn) use($that) {
            $that->emit('error', array($e));
            // parsing failed => assume dummy request and send appropriate error
            $that->writeError($conn, $e->getCode() !== 0 ? $e->getCode() : 400, new \TenantCloud\BetterReflection\Relocated\React\Http\Message\ServerRequest('GET', '/'));
        });
    }
    /**
     ...
     */
    public function listen(\TenantCloud\BetterReflection\Relocated\React\Socket\ServerInterface $socket)
    {
        $socket->on('connection', array($this->parser, 'handle'));
    }
    /** @internal */
    public function handleRequest(\TenantCloud\BetterReflection\Relocated\React\Socket\ConnectionInterface $conn, \TenantCloud\BetterReflection\Relocated\Psr\Http\Message\ServerRequestInterface $request)
    {
        ...
        try {
            ...
            $response = \TenantCloud\BetterReflection\Relocated\React\Promise\reject($error);
        ...
        if ($response instanceof \TenantCloud\BetterReflection\Relocated\React\Promise\CancellablePromiseInterface) {
            $conn->on('close', function () use($response) {
                $response->cancel();
            });
        }
        // happy path: response returned, handle and return immediately
        if ($response instanceof \TenantCloud\BetterReflection\Relocated\Psr\Http\Message\ResponseInterface) {
        ...
        if (!$response instanceof \TenantCloud\BetterReflection\Relocated\React\Promise\PromiseInterface) {
            $response = \TenantCloud\BetterReflection\Relocated\React\Promise\resolve($response);
        }
        $that = $this;
    ...
    /** @internal */
    public function writeError(\TenantCloud\BetterReflection\Relocated\React\Socket\ConnectionInterface $conn, $code, \TenantCloud\BetterReflection\Relocated\Psr\Http\Message\ServerRequestInterface $request)
    {
        $response = new \TenantCloud\BetterReflection\Relocated\React\Http\Message\Response($code, array('Content-Type' => 'text/plain'), 'Error ' . $code);
    ...
    /** @internal */
    public function handleResponse(\TenantCloud\BetterReflection\Relocated\React\Socket\ConnectionInterface $connection, \TenantCloud\BetterReflection\Relocated\Psr\Http\Message\ServerRequestInterface $request, \TenantCloud\BetterReflection\Relocated\Psr\Http\Message\ResponseInterface $response)
    {
        ...
        if (($code === 101 || $method === 'CONNECT' && $code >= 200 && $code < 300) && $body instanceof \TenantCloud\BetterReflection\Relocated\React\Http\Io\HttpBodyStream && $body->input instanceof \TenantCloud\BetterReflection\Relocated\React\Stream\WritableStreamInterface) {
        ...
        if (!$body instanceof \TenantCloud\BetterReflection\Relocated\React\Stream\ReadableStreamInterface || !$body->isReadable()) {
            ...
            if ($body instanceof \TenantCloud\BetterReflection\Relocated\React\Stream\ReadableStreamInterface && $chunked) {
                $body = "0\r\n\r\n";
            }
