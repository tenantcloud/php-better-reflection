 ...
 * @internal
 */
class RequestHeaderParser extends \TenantCloud\BetterReflection\Relocated\Evenement\EventEmitter
{
    private $maxSize = 8192;
    public function handle(\TenantCloud\BetterReflection\Relocated\React\Socket\ConnectionInterface $conn)
    {
        $buffer = '';
        ...
        $conn->on('data', $fn = function ($data) use(&$buffer, &$fn, $conn, $maxSize, $that) {
            ...
            if ($contentLength === 0) {
                ...
                $stream = new \TenantCloud\BetterReflection\Relocated\React\Http\Io\EmptyBodyStream();
                $request = $request->withBody($stream);
            } else {
                // otherwise body is present => delimit using Content-Length or ChunkedDecoder
                $stream = new \TenantCloud\BetterReflection\Relocated\React\Http\Io\CloseProtectionStream($conn);
                if ($contentLength !== null) {
                    $stream = new \TenantCloud\BetterReflection\Relocated\React\Http\Io\LengthLimitedStream($stream, $contentLength);
                } else {
                    $stream = new \TenantCloud\BetterReflection\Relocated\React\Http\Io\ChunkedDecoder($stream);
                }
                $request = $request->withBody(new \TenantCloud\BetterReflection\Relocated\React\Http\Io\HttpBodyStream($stream, $contentLength));
            }
    ...
    public function parseRequest($headers, $remoteSocketUri, $localSocketUri)
    {
        ...
        $request = new \TenantCloud\BetterReflection\Relocated\React\Http\Message\ServerRequest($start['method'], $uri, $fields, '', $start['version'], $serverParams);
