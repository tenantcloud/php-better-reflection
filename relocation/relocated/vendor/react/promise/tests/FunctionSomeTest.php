...
use TenantCloud\BetterReflection\Relocated\React\Promise\Exception\LengthException;
class FunctionSomeTest extends \TenantCloud\BetterReflection\Relocated\React\Promise\TestCase
{
    /** @test */
    public function shouldRejectWithLengthExceptionWithEmptyInputArray()
    {
        ...
        $mock->expects($this->once())->method('__invoke')->with($this->callback(function ($exception) {
            return $exception instanceof \TenantCloud\BetterReflection\Relocated\React\Promise\Exception\LengthException && 'Input array must contain at least 1 item but contains only 0 items.' === $exception->getMessage();
        }));
    ...
    public function shouldRejectWithLengthExceptionWithInputArrayContainingNotEnoughItems()
    {
        $mock = $this->createCallableMock();
        $mock->expects($this->once())->method('__invoke')->with($this->callback(function ($exception) {
            return $exception instanceof \TenantCloud\BetterReflection\Relocated\React\Promise\Exception\LengthException && 'Input array must contain at least 4 items but contains only 3 items.' === $exception->getMessage();
        }));
    ...
    public function shouldResolveToEmptyArrayWithNonArrayInput()
    {
        ...
        some(null, 1)->then($mock);
    }
    /** @test */
    public function shouldResolveValuesArray()
    {
        $mock = $this->createCallableMock();
        $mock->expects($this->once())->method('__invoke')->with($this->identicalTo([1, 2]));
        some([1, 2, 3], 2)->then($mock);
    }
    /** @test */
    public function shouldResolvePromisesArray()
    {
        $mock = $this->createCallableMock();
        $mock->expects($this->once())->method('__invoke')->with($this->identicalTo([1, 2]));
        some([resolve(1), resolve(2), resolve(3)], 2)->then($mock);
    }
    /** @test */
    public function shouldResolveSparseArrayInput()
    {
        $mock = $this->createCallableMock();
        $mock->expects($this->once())->method('__invoke')->with($this->identicalTo([null, 1]));
        some([null, 1, null, 2, 3], 2)->then($mock);
    }
    /** @test */
    ...
    public function shouldAcceptAPromiseForAnArray()
    {
        $mock = $this->createCallableMock();
        $mock->expects($this->once())->method('__invoke')->with($this->identicalTo([1, 2]));
        some(resolve([1, 2, 3]), 2)->then($mock);
    }
    /** @test */
    ...
    public function shouldRejectWhenInputPromiseRejects()
    {
        ...
        some(reject(), 1)->then($this->expectCallableNever(), $mock);
    }
    /** @test */
