...
class PromiseTest extends \TenantCloud\BetterReflection\Relocated\React\Promise\TestCase
{
    use PromiseTest\FullTestTrait;
    public function getPromiseTestAdapter(callable $canceller = null)
    {
        $resolveCallback = $rejectCallback = $progressCallback = null;
        $promise = new \TenantCloud\BetterReflection\Relocated\React\Promise\Promise(function ($resolve, $reject, $progress) use(&$resolveCallback, &$rejectCallback, &$progressCallback) {
        ...
        return new \TenantCloud\BetterReflection\Relocated\React\Promise\PromiseAdapter\CallbackPromiseAdapter(['promise' => function () use($promise) {
            return $promise;
        }, 'resolve' => $resolveCallback, 'reject' => $rejectCallback, 'notify' => $progressCallback, 'settle' => $resolveCallback]);
    }
    /** @test */
    public function shouldRejectIfResolverThrowsException()
    {
        $exception = new \Exception('foo');
        $promise = new \TenantCloud\BetterReflection\Relocated\React\Promise\Promise(function () use($exception) {
            throw $exception;
        });
    ...
    public function shouldResolveWithoutCreatingGarbageCyclesIfResolverResolvesWithException()
    {
        ...
        $promise = new \TenantCloud\BetterReflection\Relocated\React\Promise\Promise(function ($resolve) {
    ...
    {
        \gc_collect_cycles();
        $promise = new \TenantCloud\BetterReflection\Relocated\React\Promise\Promise(function ($resolve, $reject) {
    ...
    {
        \gc_collect_cycles();
        $promise = new \TenantCloud\BetterReflection\Relocated\React\Promise\Promise(function () use(&$promise) {
            throw new \Exception('foo');
        });
        unset($promise);
        $this->assertSame(0, \gc_collect_cycles());
    ...
    public function shouldFulfillIfFullfilledWithSimplePromise()
    {
        ...
        $adapter->resolve(new \TenantCloud\BetterReflection\Relocated\React\Promise\SimpleFulfilledTestPromise());
    }
    /** @test */
    public function shouldRejectIfRejectedWithSimplePromise()
    {
        ...
        $adapter->resolve(new \TenantCloud\BetterReflection\Relocated\React\Promise\SimpleRejectedTestPromise());
    }
}
