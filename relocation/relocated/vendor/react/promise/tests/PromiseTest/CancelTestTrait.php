...
trait CancelTestTrait
{
    ...
    public function cancelShouldRejectPromiseWithExceptionIfCancellerThrows()
    {
        ...
        $mock = $this->createCallableMock();
        $mock->expects($this->once())->method('__invoke')->with($this->identicalTo($e));
        $adapter->promise()->then($this->expectCallableNever(), $mock);
        $adapter->promise()->cancel();
    }
    /** @test */
    ...
    {
        $mock = $this->createCallableMock();
        $mock->expects($this->once())->method('__invoke')->will($this->returnCallback(function ($resolve) {
            $resolve();
        }));
    ...
    public function cancelShouldCallCancellerFromDeepNestedPromiseChain()
    {
        ...
        $promise = $adapter->promise()->then(function () {
            return new \TenantCloud\BetterReflection\Relocated\React\Promise\Promise(function () {
            });
        })->then(function () {
    ...
    public function cancelCalledOnChildrenSouldOnlyCancelWhenAllChildrenCancelled()
    {
        ...
        $child1 = $adapter->promise()->then()->then();
