...
trait ResolveTestTrait
{
    ...
    public function resolveShouldResolveWithPromisedValue()
    {
        ...
        $mock->expects($this->once())->method('__invoke')->with($this->identicalTo(1));
        $adapter->promise()->then($mock);
        $adapter->resolve(\TenantCloud\BetterReflection\Relocated\React\Promise\resolve(1));
    }
    /** @test */
    public function resolveShouldRejectWhenResolvedWithRejectedPromise()
    {
        ...
        $adapter->resolve(\TenantCloud\BetterReflection\Relocated\React\Promise\reject(1));
    }
    /** @test */
    ...
    public function resolveShouldMakePromiseImmutable()
    {
        ...
        $mock = $this->createCallableMock();
        $mock->expects($this->once())->method('__invoke')->with($this->identicalTo(1));
        $adapter->promise()->then(function ($value) use($adapter) {
            $adapter->resolve(3);
            return $value;
    ...
    public function resolveShouldRejectWhenResolvedWithItself()
    {
        $adapter = $this->getPromiseTestAdapter();
        $mock = $this->createCallableMock();
        $mock->expects($this->once())->method('__invoke')->with(new \LogicException('Cannot resolve a promise with itself.'));
        $adapter->promise()->then($this->expectCallableNever(), $mock);
    ...
    public function resolveShouldRejectWhenResolvedWithAPromiseWhichFollowsItself()
    {
        ...
        $promise2->then($this->expectCallableNever(), $mock);
