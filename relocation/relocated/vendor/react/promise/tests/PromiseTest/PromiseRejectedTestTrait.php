...
use TenantCloud\BetterReflection\Relocated\React\Promise\UnhandledRejectionException;
trait PromiseRejectedTestTrait
{
    ...
    public function shouldForwardUndefinedRejectionValue()
    {
        $adapter = $this->getPromiseTestAdapter();
        $mock = $this->createCallableMock();
        $mock->expects($this->once())->method('__invoke')->with(null);
        $adapter->reject(1);
        $adapter->promise()->then($this->expectCallableNever(), function () {
    ...
    public function shouldSwitchFromErrbacksToCallbacksWhenErrbackReturnsAResolution()
    {
        ...
        $adapter->reject(1);
        $adapter->promise()->then($this->expectCallableNever(), function ($val) {
            return \TenantCloud\BetterReflection\Relocated\React\Promise\resolve($val + 1);
        })->then($mock, $this->expectCallableNever());
    }
    /** @test */
    public function shouldPropagateRejectionsWhenErrbackThrows()
    {
        ...
        $mock = $this->createCallableMock();
        $mock->expects($this->once())->method('__invoke')->will($this->throwException($exception));
        $mock2 = $this->createCallableMock();
        $mock2->expects($this->once())->method('__invoke')->with($this->identicalTo($exception));
        $adapter->reject(1);
        $adapter->promise()->then($this->expectCallableNever(), $mock)->then($this->expectCallableNever(), $mock2);
    }
    /** @test */
    public function shouldPropagateRejectionsWhenErrbackReturnsARejection()
    {
        ...
        $adapter->reject(1);
        $adapter->promise()->then($this->expectCallableNever(), function ($val) {
            return \TenantCloud\BetterReflection\Relocated\React\Promise\reject($val + 1);
    ...
    public function unhandledRejectionExceptionThrownByDoneHoldsRejectionValue()
    {
        ...
        } catch (\TenantCloud\BetterReflection\Relocated\React\Promise\UnhandledRejectionException $e) {
    ...
    public function otherwiseShouldInvokeNonTypeHintedRejectionHandlerIfReasonIsAnExceptionForRejectedPromise()
    {
        ...
        $adapter->promise()->otherwise(function ($reason) use($mock) {
            $mock($reason);
        });
    }
    /** @test */
    public function otherwiseShouldInvokeRejectionHandlerIfReasonMatchesTypehintForRejectedPromise()
    {
        ...
        $mock->expects($this->once())->method('__invoke')->with($this->identicalTo($exception));
        $adapter->reject($exception);
        $adapter->promise()->otherwise(function (\InvalidArgumentException $reason) use($mock) {
            $mock($reason);
        });
    }
    /** @test */
    ...
    public function alwaysShouldRejectWhenHandlerRejectsForRejectedPromise()
    {
        ...
        $adapter->reject($exception1);
        $adapter->promise()->always(function () use($exception2) {
            return \TenantCloud\BetterReflection\Relocated\React\Promise\reject($exception2);
        })->then(null, $mock);
    }
    /** @test */
