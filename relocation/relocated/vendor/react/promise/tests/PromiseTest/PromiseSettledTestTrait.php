...
trait PromiseSettledTestTrait
{
    ...
    public function thenShouldReturnAPromiseForSettledPromise()
    {
        ...
        $adapter->settle();
        $this->assertInstanceOf('TenantCloud\\BetterReflection\\Relocated\\React\\Promise\\PromiseInterface', $adapter->promise()->then());
    }
    /** @test */
    public function thenShouldReturnAllowNullForSettledPromise()
    {
        $adapter = $this->getPromiseTestAdapter();
        $adapter->settle();
        $this->assertInstanceOf('TenantCloud\\BetterReflection\\Relocated\\React\\Promise\\PromiseInterface', $adapter->promise()->then(null, null, null));
    }
    /** @test */
    ...
    public function alwaysShouldReturnAPromiseForSettledPromise()
    {
        $adapter = $this->getPromiseTestAdapter();
        $adapter->settle();
        $this->assertInstanceOf('TenantCloud\\BetterReflection\\Relocated\\React\\Promise\\PromiseInterface', $adapter->promise()->always(function () {
        }));
    }
}
