...
trait NotifyTestTrait
{
    ...
    public function notifyShouldProgress()
    {
        ...
        $adapter->promise()->then($this->expectCallableNever(), $this->expectCallableNever(), $mock);
    ...
    public function notifyShouldPropagateProgressToDownstreamPromises()
    {
        ...
        $sentinel = new \stdClass();
        $mock = $this->createCallableMock();
        $mock->expects($this->once())->method('__invoke')->will($this->returnArgument(0));
        $mock2 = $this->createCallableMock();
        $mock2->expects($this->once())->method('__invoke')->with($sentinel);
        $adapter->promise()->then($this->expectCallableNever(), $this->expectCallableNever(), $mock)->then($this->expectCallableNever(), $this->expectCallableNever(), $mock2);
        $adapter->notify($sentinel);
    }
    /** @test */
    public function notifyShouldPropagateTransformedProgressToDownstreamPromises()
    {
        ...
        $sentinel = new \stdClass();
        $mock = $this->createCallableMock();
        $mock->expects($this->once())->method('__invoke')->will($this->returnValue($sentinel));
        $mock2 = $this->createCallableMock();
        $mock2->expects($this->once())->method('__invoke')->with($sentinel);
        $adapter->promise()->then($this->expectCallableNever(), $this->expectCallableNever(), $mock)->then($this->expectCallableNever(), $this->expectCallableNever(), $mock2);
        $adapter->notify(1);
    ...
    public function notifyShouldForwardProgressEventsWhenIntermediaryCallbackTiedToAResolvedPromiseReturnsAPromise()
    {
        ...
        $adapter->resolve();
        $adapter->promise()->then(function () use($promise2) {
            return $promise2;
    ...
    public function notifyShouldAllowResolveAfterProgress()
    {
        $adapter = $this->getPromiseTestAdapter();
        $mock = $this->createCallableMock();
        $mock->expects($this->exactly(2))->method('__invoke')->withConsecutive(array($this->identicalTo(1)), array($this->identicalTo(2)));
        $adapter->promise()->then($mock, $this->expectCallableNever(), $mock);
        $adapter->notify(1);
        $adapter->resolve(2);
    }
    /** @test */
    public function notifyShouldAllowRejectAfterProgress()
    {
        ...
        $mock = $this->createCallableMock();
        $mock->expects($this->exactly(2))->method('__invoke')->withConsecutive(array($this->identicalTo(1)), array($this->identicalTo(2)));
        $adapter->promise()->then($this->expectCallableNever(), $mock, $mock);
        $adapter->notify(1);
        $adapter->reject(2);
    }
    /** @test */
