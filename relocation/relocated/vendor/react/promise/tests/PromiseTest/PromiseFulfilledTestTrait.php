...
trait PromiseFulfilledTestTrait
{
    ...
    public function thenShouldForwardResultWhenCallbackIsNull()
    {
        ...
        $mock->expects($this->once())->method('__invoke')->with($this->identicalTo(1));
        $adapter->resolve(1);
        $adapter->promise()->then(null, $this->expectCallableNever())->then($mock, $this->expectCallableNever());
    }
    /** @test */
    ...
    public function thenShouldSwitchFromCallbacksToErrbacksWhenCallbackThrows()
    {
        ...
        $adapter->resolve(1);
        $adapter->promise()->then($mock, $this->expectCallableNever())->then($this->expectCallableNever(), $mock2);
    }
    /** @test */
