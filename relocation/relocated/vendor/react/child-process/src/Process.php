...
namespace TenantCloud\BetterReflection\Relocated\React\ChildProcess;
...
use TenantCloud\BetterReflection\Relocated\React\Stream\ReadableResourceStream;
use TenantCloud\BetterReflection\Relocated\React\Stream\ReadableStreamInterface;
use TenantCloud\BetterReflection\Relocated\React\Stream\WritableResourceStream;
use TenantCloud\BetterReflection\Relocated\React\Stream\WritableStreamInterface;
use TenantCloud\BetterReflection\Relocated\React\Stream\DuplexResourceStream;
use TenantCloud\BetterReflection\Relocated\React\Stream\DuplexStreamInterface;
/**
 * Process component.
 *
 ...
 */
class Process extends \TenantCloud\BetterReflection\Relocated\Evenement\EventEmitter
{
    /**
     ...
     */
    public function start(\TenantCloud\BetterReflection\Relocated\React\EventLoop\LoopInterface $loop, $interval = 0.1)
    {
        if ($this->isRunning()) {
        ...
        foreach ($pipes as $n => $fd) {
            ...
            if ($mode === 'r+') {
                $stream = new \TenantCloud\BetterReflection\Relocated\React\Stream\DuplexResourceStream($fd, $loop);
            } elseif ($mode === 'w') {
                $stream = new \TenantCloud\BetterReflection\Relocated\React\Stream\WritableResourceStream($fd, $loop);
            } else {
                $stream = new \TenantCloud\BetterReflection\Relocated\React\Stream\ReadableResourceStream($fd, $loop);
