...
use TenantCloud\BetterReflection\Relocated\Evenement\EventEmitterInterface;
 ...
 */
function buffer(\TenantCloud\BetterReflection\Relocated\React\Stream\ReadableStreamInterface $stream, $maxLength = null)
{
    // stream already ended => resolve with empty buffer
    if (!$stream->isReadable()) {
        return \TenantCloud\BetterReflection\Relocated\React\Promise\resolve('');
    }
    $buffer = '';
    $promise = new \TenantCloud\BetterReflection\Relocated\React\Promise\Promise(function ($resolve, $reject) use($stream, $maxLength, &$buffer, &$bufferer) {
 ...
 */
function first(\TenantCloud\BetterReflection\Relocated\Evenement\EventEmitterInterface $stream, $event = 'data')
{
    if ($stream instanceof \TenantCloud\BetterReflection\Relocated\React\Stream\ReadableStreamInterface) {
        ...
        if (!$stream->isReadable()) {
            return \TenantCloud\BetterReflection\Relocated\React\Promise\reject(new \RuntimeException('Stream already closed'));
        }
    } elseif ($stream instanceof \TenantCloud\BetterReflection\Relocated\React\Stream\WritableStreamInterface) {
        // writable-only stream (not duplex) not writable => already closed
        if (!$stream->isWritable()) {
            return \TenantCloud\BetterReflection\Relocated\React\Promise\reject(new \RuntimeException('Stream already closed'));
        }
    }
    return new \TenantCloud\BetterReflection\Relocated\React\Promise\Promise(function ($resolve, $reject) use($stream, $event, &$listener) {
 ...
 */
function all(\TenantCloud\BetterReflection\Relocated\Evenement\EventEmitterInterface $stream, $event = 'data')
{
    // stream already ended => resolve with empty buffer
    if ($stream instanceof \TenantCloud\BetterReflection\Relocated\React\Stream\ReadableStreamInterface) {
        // readable or duplex stream not readable => already closed
        // a half-open duplex stream is considered closed if its readable side is closed
        if (!$stream->isReadable()) {
            return \TenantCloud\BetterReflection\Relocated\React\Promise\resolve(array());
        }
    } elseif ($stream instanceof \TenantCloud\BetterReflection\Relocated\React\Stream\WritableStreamInterface) {
        // writable-only stream (not duplex) not writable => already closed
        if (!$stream->isWritable()) {
    ...
    $promise = new \TenantCloud\BetterReflection\Relocated\React\Promise\Promise(function ($resolve, $reject) use($stream, &$buffer) {
        $stream->on('error', function ($error) use($reject) {
            $reject(new \RuntimeException('An error occured on the underlying stream while buffering', 0, $error));
        });
        $stream->on('close', function () use($resolve, &$buffer) {
 ...
 */
function unwrapReadable(\TenantCloud\BetterReflection\Relocated\React\Promise\PromiseInterface $promise)
{
    return new \TenantCloud\BetterReflection\Relocated\React\Promise\Stream\UnwrapReadableStream($promise);
}
/**
 ...
 */
function unwrapWritable(\TenantCloud\BetterReflection\Relocated\React\Promise\PromiseInterface $promise)
{
    return new \TenantCloud\BetterReflection\Relocated\React\Promise\Stream\UnwrapWritableStream($promise);
}
