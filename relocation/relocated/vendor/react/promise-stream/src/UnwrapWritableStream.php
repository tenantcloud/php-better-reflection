 ...
 */
class UnwrapWritableStream extends \TenantCloud\BetterReflection\Relocated\Evenement\EventEmitter implements \TenantCloud\BetterReflection\Relocated\React\Stream\WritableStreamInterface
{
    private $promise;
    private $stream;
    ...
    public function __construct(\TenantCloud\BetterReflection\Relocated\React\Promise\PromiseInterface $promise)
    {
        ...
        $this->promise = $promise->then(function ($stream) {
            if (!$stream instanceof \TenantCloud\BetterReflection\Relocated\React\Stream\WritableStreamInterface) {
                throw new \InvalidArgumentException('Not a writable stream');
            }
            return $stream;
        })->then(function (\TenantCloud\BetterReflection\Relocated\React\Stream\WritableStreamInterface $stream) use($out, &$store, &$buffer, &$ending, &$closed) {
