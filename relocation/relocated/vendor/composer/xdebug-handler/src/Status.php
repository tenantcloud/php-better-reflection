...
class Status
{
    ...
    public function __construct($envAllowXdebug, $debug)
    {
        ...
        \TenantCloud\BetterReflection\Relocated\Composer\XdebugHandler\Process::setEnv(self::ENV_RESTART);
    ...
    {
        if ($this->logger) {
            $this->logger->log($level ?: \TenantCloud\BetterReflection\Relocated\Psr\Log\LogLevel::DEBUG, $text);
        }
        if ($this->debug) {
    ...
    {
        $this->output(\sprintf('No restart (%s)', $error), \TenantCloud\BetterReflection\Relocated\Psr\Log\LogLevel::WARNING);
    }
    ...
    {
        $this->output($this->getLoadedMessage());
        \TenantCloud\BetterReflection\Relocated\Composer\XdebugHandler\Process::setEnv(self::ENV_RESTART, (string) \microtime(\true));
    }
    private function reportRestarted()
    {
        $loaded = $this->getLoadedMessage();
        $text = \sprintf('Restarted (%d ms). %s', $this->time, $loaded);
        $level = $this->loaded ? \TenantCloud\BetterReflection\Relocated\Psr\Log\LogLevel::WARNING : null;
