...
use Error;
use TenantCloud\BetterReflection\Relocated\JetBrains\PHPStormStub\PhpStormStubsMap;
use ParseError;
...
use TenantCloud\BetterReflection\Relocated\PHPStan\BetterReflection\SourceLocator\SourceStubber\Exception\CouldNotFindPhpStormStubs;
 ...
 * @internal
 */
final class PhpStormStubsSourceStubber implements \TenantCloud\BetterReflection\Relocated\PHPStan\BetterReflection\SourceLocator\SourceStubber\SourceStubber
{
    ...
    /** @var array<lowercase-string, string> */
    private $constantMap;
    public function __construct(\TenantCloud\BetterReflection\Relocated\PhpParser\Parser $phpParser, ?int $phpVersionId = null)
    {
        ...
        $this->classMap = \array_change_key_case(\TenantCloud\BetterReflection\Relocated\JetBrains\PHPStormStub\PhpStormStubsMap::CLASSES);
        $this->functionMap = \array_change_key_case(\TenantCloud\BetterReflection\Relocated\JetBrains\PHPStormStub\PhpStormStubsMap::FUNCTIONS);
        $this->constantMap = \TenantCloud\BetterReflection\Relocated\JetBrains\PHPStormStub\PhpStormStubsMap::CONSTANTS;
    }
    ...
    public function generateClassStub(string $className) : ?\TenantCloud\BetterReflection\Relocated\PHPStan\BetterReflection\SourceLocator\SourceStubber\StubData
    {
        ...
        if ($classNode instanceof \TenantCloud\BetterReflection\Relocated\PhpParser\Node\Stmt\Class_) {
            if ($classNode->name instanceof \TenantCloud\BetterReflection\Relocated\PhpParser\Node\Identifier && $classNode->name->toString() === \ParseError::class && $this->phpVersionId < 70300) {
                $classNode->extends = new \TenantCloud\BetterReflection\Relocated\PhpParser\Node\Name\FullyQualified(\Error::class);
        ...
        } elseif ($classNode instanceof \TenantCloud\BetterReflection\Relocated\PhpParser\Node\Stmt\Interface_) {
        ...
        return new \TenantCloud\BetterReflection\Relocated\PHPStan\BetterReflection\SourceLocator\SourceStubber\StubData($stub, $this->getExtensionFromFilePath($filePath), $this->getAbsoluteFilePath($filePath));
    }
    private function findClassNode(string $filePath, string $lowercaseName) : ?\TenantCloud\BetterReflection\Relocated\PhpParser\Node\Stmt\ClassLike
    {
    ...
    public function generateFunctionStub(string $functionName) : ?\TenantCloud\BetterReflection\Relocated\PHPStan\BetterReflection\SourceLocator\SourceStubber\StubData
    {
        ...
        return new \TenantCloud\BetterReflection\Relocated\PHPStan\BetterReflection\SourceLocator\SourceStubber\StubData($this->createStub($functionNode), $this->getExtensionFromFilePath($filePath));
    }
    private function findFunctionNode(string $filePath, string $lowercaseFunctionName) : ?\TenantCloud\BetterReflection\Relocated\PhpParser\Node\Stmt\Function_
    {
    ...
    public function generateConstantStub(string $constantName) : ?\TenantCloud\BetterReflection\Relocated\PHPStan\BetterReflection\SourceLocator\SourceStubber\StubData
    {
        ...
        return new \TenantCloud\BetterReflection\Relocated\PHPStan\BetterReflection\SourceLocator\SourceStubber\StubData($this->createStub($constantNode), $this->getExtensionFromFilePath($filePath));
    }
    private function parseFile(string $filePath) : void
    {
        $absoluteFilePath = $this->getAbsoluteFilePath($filePath);
        \TenantCloud\BetterReflection\Relocated\PHPStan\BetterReflection\SourceLocator\FileChecker::assertReadableFile($absoluteFilePath);
        ...
        foreach ($this->cachingVisitor->getClassNodes() as $className => $classNode) {
            ...
            \assert($classNode instanceof \TenantCloud\BetterReflection\Relocated\PhpParser\Node\Stmt\ClassLike);
        ...
        foreach ($this->cachingVisitor->getFunctionNodes() as $functionName => $functionNode) {
            ...
            \assert($functionNode instanceof \TenantCloud\BetterReflection\Relocated\PhpParser\Node\Stmt\Function_);
        ...
        foreach ($this->cachingVisitor->getConstantNodes() as $constantName => $constantNode) {
            ...
            \assert($constantNode instanceof \TenantCloud\BetterReflection\Relocated\PhpParser\Node\Stmt\Const_ || $constantNode instanceof \TenantCloud\BetterReflection\Relocated\PhpParser\Node\Expr\FuncCall);
    ...
    private function hasLaterSinceVersion(\TenantCloud\BetterReflection\Relocated\PhpParser\Node $node) : bool
    {
     ...
     */
    private function createStub(\TenantCloud\BetterReflection\Relocated\PhpParser\Node $node) : string
    {
        $nodeWithNamespaceName = $node instanceof \TenantCloud\BetterReflection\Relocated\PhpParser\Node\Stmt\Const_ ? $node->consts[0] : $node;
        ...
        return "<?php\n\n" . $printed . ($node instanceof \TenantCloud\BetterReflection\Relocated\PhpParser\Node\Expr\FuncCall ? ';' : '') . "\n";
    }
    private function createCachingVisitor() : \TenantCloud\BetterReflection\Relocated\PhpParser\NodeVisitorAbstract
    {
    ...
    private function getStubsDirectory() : string
    {
        ...
        throw \TenantCloud\BetterReflection\Relocated\PHPStan\BetterReflection\SourceLocator\SourceStubber\Exception\CouldNotFindPhpStormStubs::create();
    }
}
