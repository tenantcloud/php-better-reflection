...
use TenantCloud\BetterReflection\Relocated\PHPStan\Dependency\ExportedNodeFetcher;
...
class ResultCacheManager
{
    ...
    private \TenantCloud\BetterReflection\Relocated\PHPStan\Dependency\ExportedNodeFetcher $exportedNodeFetcher;
    private \TenantCloud\BetterReflection\Relocated\PHPStan\File\FileFinder $scanFileFinder;
     ...
     */
    public function __construct(\TenantCloud\BetterReflection\Relocated\PHPStan\Dependency\ExportedNodeFetcher $exportedNodeFetcher, \TenantCloud\BetterReflection\Relocated\PHPStan\File\FileFinder $scanFileFinder, string $cacheFilePath, string $tempResultCachePath, array $analysedPaths, array $composerAutoloaderProjectPaths, array $stubFiles, string $usedLevel, ?string $cliAutoloadFile, array $bootstrapFiles, array $scanFiles, array $scanDirectories, array $fileReplacements)
    {
     ...
     * @return ResultCache
     */
    public function restore(array $allAnalysedFiles, bool $debug, bool $onlyFiles, ?array $projectConfigArray, \TenantCloud\BetterReflection\Relocated\PHPStan\Command\Output $output, ?string $resultCacheName = null) : \TenantCloud\BetterReflection\Relocated\PHPStan\Analyser\ResultCache\ResultCache
    {
        if ($debug) {
            if ($output->isDebug()) {
                $output->writeLineFormatted('Result cache not used because of debug mode.');
            }
            return new \TenantCloud\BetterReflection\Relocated\PHPStan\Analyser\ResultCache\ResultCache($allAnalysedFiles, \true, \time(), $this->getMeta($allAnalysedFiles, $projectConfigArray), [], [], []);
        }
        if ($onlyFiles) {
            if ($output->isDebug()) {
        ...
        if ($data['meta'] !== $meta) {
            ...
            return new \TenantCloud\BetterReflection\Relocated\PHPStan\Analyser\ResultCache\ResultCache($allAnalysedFiles, \true, \time(), $meta, [], [], []);
        }
        ...
        return new \TenantCloud\BetterReflection\Relocated\PHPStan\Analyser\ResultCache\ResultCache(\array_unique($filesToAnalyse), \false, $data['lastFullAnalysisTime'], $meta, $filteredErrors, $invertedDependenciesToReturn, $filteredExportedNodes);
    }
    /**
     ...
     */
    public function process(\TenantCloud\BetterReflection\Relocated\PHPStan\Analyser\AnalyserResult $analyserResult, \TenantCloud\BetterReflection\Relocated\PHPStan\Analyser\ResultCache\ResultCache $resultCache, \TenantCloud\BetterReflection\Relocated\PHPStan\Command\Output $output, bool $onlyFiles, $save) : \TenantCloud\BetterReflection\Relocated\PHPStan\Analyser\ResultCache\ResultCacheProcessResult
    {
        ...
        if ($resultCache->isFullAnalysis()) {
            ...
            return new \TenantCloud\BetterReflection\Relocated\PHPStan\Analyser\ResultCache\ResultCacheProcessResult($analyserResult, $saved);
        }
        ...
        return new \TenantCloud\BetterReflection\Relocated\PHPStan\Analyser\ResultCache\ResultCacheProcessResult(new \TenantCloud\BetterReflection\Relocated\PHPStan\Analyser\AnalyserResult($flatErrors, $internalErrors, $dependencies, $exportedNodes, $analyserResult->hasReachedInternalErrorsCountLimit()), $saved);
    }
    /**
     * @param ResultCache $resultCache
     ...
     */
    private function mergeErrors(\TenantCloud\BetterReflection\Relocated\PHPStan\Analyser\ResultCache\ResultCache $resultCache, array $freshErrorsByFile) : array
    {
     ...
     */
    private function mergeDependencies(\TenantCloud\BetterReflection\Relocated\PHPStan\Analyser\ResultCache\ResultCache $resultCache, ?array $freshDependencies) : ?array
    {
     ...
     */
    private function mergeExportedNodes(\TenantCloud\BetterReflection\Relocated\PHPStan\Analyser\ResultCache\ResultCache $resultCache, array $freshExportedNodes) : array
    {
     ...
     * @param mixed[] $meta
     */
    private function save(int $lastFullAnalysisTime, ?string $resultCacheName, array $errors, array $dependencies, array $exportedNodes, array $meta) : void
    {
...
php;
        ...
        \TenantCloud\BetterReflection\Relocated\PHPStan\File\FileWriter::write($file, \sprintf($template, \var_export($lastFullAnalysisTime, \true), \var_export($meta, \true), \var_export($errors, \true), \var_export($invertedDependencies, \true), \var_export($exportedNodes, \true)));
    }
    /**
     * @param string[] $allAnalysedFiles
     * @param mixed[]|null $projectConfigArray
    ...
    private function getMeta(array $allAnalysedFiles, ?array $projectConfigArray) : array
    {
        ...
        if ($projectConfigArray !== null) {
            ...
            $projectConfigArray = \TenantCloud\BetterReflection\Relocated\Nette\Neon\Neon::encode($projectConfigArray);
        }
