...
class RunnableQueue
{
    private \TenantCloud\BetterReflection\Relocated\PHPStan\Process\Runnable\RunnableQueueLogger $logger;
    private int $maxSize;
    /** @var array<array{Runnable, int, Deferred}> */
    private array $queue = [];
    /** @var SplObjectStorage<Runnable, array{int, Deferred}> */
    private \SplObjectStorage $running;
    public function __construct(\TenantCloud\BetterReflection\Relocated\PHPStan\Process\Runnable\RunnableQueueLogger $logger, int $maxSize)
    {
        $this->logger = $logger;
    ...
    public function queue(\TenantCloud\BetterReflection\Relocated\PHPStan\Process\Runnable\Runnable $runnable, int $size) : \TenantCloud\BetterReflection\Relocated\React\Promise\CancellablePromiseInterface
    {
        if ($size > $this->maxSize) {
            throw new \TenantCloud\BetterReflection\Relocated\PHPStan\ShouldNotHappenException('Runnable size exceeds queue maxSize.');
        }
        $deferred = new \TenantCloud\BetterReflection\Relocated\React\Promise\Deferred(static function () use($runnable) : void {
            $runnable->cancel();
        });
    ...
    private function drainQueue() : void
    {
        ...
        if ($currentQueueSize > $this->maxSize) {
            throw new \TenantCloud\BetterReflection\Relocated\PHPStan\ShouldNotHappenException('Running overflow');
        }
        if ($currentQueueSize === $this->maxSize) {
