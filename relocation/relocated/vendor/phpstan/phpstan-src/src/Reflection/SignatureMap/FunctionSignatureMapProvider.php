...
class FunctionSignatureMapProvider implements \TenantCloud\BetterReflection\Relocated\PHPStan\Reflection\SignatureMap\SignatureMapProvider
{
    private \TenantCloud\BetterReflection\Relocated\PHPStan\Reflection\SignatureMap\SignatureMapParser $parser;
    ...
    public function __construct(\TenantCloud\BetterReflection\Relocated\PHPStan\Reflection\SignatureMap\SignatureMapParser $parser, \TenantCloud\BetterReflection\Relocated\PHPStan\Php\PhpVersion $phpVersion)
    {
        $this->parser = $parser;
    ...
    public function getMethodSignature(string $className, string $methodName, ?\ReflectionMethod $reflectionMethod, int $variant = 0) : \TenantCloud\BetterReflection\Relocated\PHPStan\Reflection\SignatureMap\FunctionSignature
    {
        ...
        foreach ($signature->getParameters() as $i => $parameter) {
            ...
            $parameters[] = new \TenantCloud\BetterReflection\Relocated\PHPStan\Reflection\SignatureMap\ParameterSignature($parameter->getName(), $parameter->isOptional(), $parameter->getType(), \TenantCloud\BetterReflection\Relocated\PHPStan\Type\TypehintHelper::decideTypeFromReflection($nativeParameters[$i]->getType()), $parameter->passedByReference(), $parameter->isVariadic());
        }
        if ($reflectionMethod === null) {
            $nativeReturnType = new \TenantCloud\BetterReflection\Relocated\PHPStan\Type\MixedType();
        } else {
            $nativeReturnType = \TenantCloud\BetterReflection\Relocated\PHPStan\Type\TypehintHelper::decideTypeFromReflection($reflectionMethod->getReturnType());
        }
        return new \TenantCloud\BetterReflection\Relocated\PHPStan\Reflection\SignatureMap\FunctionSignature($parameters, $signature->getReturnType(), $nativeReturnType, $signature->isVariadic());
    }
    ...
    public function getSignatureMap() : array
    {
        if ($this->signatureMap === null) {
            ...
            if (!\is_array($signatureMap)) {
                throw new \TenantCloud\BetterReflection\Relocated\PHPStan\ShouldNotHappenException('Signature map could not be loaded.');
            }
