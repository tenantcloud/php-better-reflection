...
final class HashHmacFunctionsReturnTypeExtension implements \TenantCloud\BetterReflection\Relocated\PHPStan\Type\DynamicFunctionReturnTypeExtension
{
    ...
    public function getTypeFromFunctionCall(\TenantCloud\BetterReflection\Relocated\PHPStan\Reflection\FunctionReflection $functionReflection, \TenantCloud\BetterReflection\Relocated\PhpParser\Node\Expr\FuncCall $functionCall, \TenantCloud\BetterReflection\Relocated\PHPStan\Analyser\Scope $scope) : \TenantCloud\BetterReflection\Relocated\PHPStan\Type\Type
    {
        if ($functionReflection->getName() === 'hash_hmac') {
            $defaultReturnType = new \TenantCloud\BetterReflection\Relocated\PHPStan\Type\StringType();
        } else {
        ...
        $string = $values[0];
        return \in_array($string->getValue(), self::HMAC_ALGORITHMS, \true) ? $defaultReturnType : new \TenantCloud\BetterReflection\Relocated\PHPStan\Type\Constant\ConstantBooleanType(\false);
    }
}
