...
class FuzzyRelativePathHelper implements \TenantCloud\BetterReflection\Relocated\PHPStan\File\RelativePathHelper
{
    private \TenantCloud\BetterReflection\Relocated\PHPStan\File\RelativePathHelper $fallbackRelativePathHelper;
     ...
     */
    public function __construct(\TenantCloud\BetterReflection\Relocated\PHPStan\File\RelativePathHelper $fallbackRelativePathHelper, string $currentWorkingDirectory, array $analysedPaths, ?string $directorySeparator = null)
    {
        ...
        foreach ($analysedPaths as $pathNumber => $path) {
            ...
            foreach ($pathArray as $i => $pathPart) {
                if (\TenantCloud\BetterReflection\Relocated\Nette\Utils\Strings::endsWith($pathPart, '.php')) {
                    continue;
                }
