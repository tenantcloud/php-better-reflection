...
use TenantCloud\BetterReflection\Relocated\Httpful\Request;
...
(function () {
    ...
    $command = new class extends \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Command\Command
    {
        protected function configure()
        {
            $this->setName('run');
            $this->addArgument('fromCommit', \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Input\InputArgument::REQUIRED);
            $this->addArgument('toCommit', \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Input\InputArgument::REQUIRED);
        }
        protected function execute(\TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Input\InputInterface $input, \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Output\OutputInterface $output)
        {
            ...
            foreach ($commits as $commit) {
                $searchPullRequestsResponse = \TenantCloud\BetterReflection\Relocated\Httpful\Request::get(\sprintf('https://api.github.com/search/issues?q=repo:phpstan/phpstan-src+%s', $commit['hash']))->sendsAndExpectsType('application/json')->basicAuth('ondrejmirtes', \getenv('GITHUB_TOKEN'))->send();
                ...
                $searchIssuesResponse = \TenantCloud\BetterReflection\Relocated\Httpful\Request::get(\sprintf('https://api.github.com/search/issues?q=repo:phpstan/phpstan+%s', $commit['hash']))->sendsAndExpectsType('application/json')->basicAuth('ondrejmirtes', \getenv('GITHUB_TOKEN'))->send();
