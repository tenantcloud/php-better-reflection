 ...
 * @group exec
 */
class ResultCacheEndToEndTest extends \TenantCloud\BetterReflection\Relocated\PHPUnit\Framework\TestCase
{
    public function setUp() : void
    {
        ...
        $baselineContents = \TenantCloud\BetterReflection\Relocated\PHPStan\File\FileReader::read($baselinePath);
    ...
    public function testResultCache() : void
    {
        ...
        $lexerCode = \TenantCloud\BetterReflection\Relocated\PHPStan\File\FileReader::read($lexerPath);
        ...
        $errorHandlerContents = \TenantCloud\BetterReflection\Relocated\PHPStan\File\FileReader::read($errorHandlerPath);
        ...
        $originalBootstrapContents = \TenantCloud\BetterReflection\Relocated\PHPStan\File\FileReader::read($bootstrapPath);
    ...
    public function testResultCacheDeleteFile() : void
    {
        ...
        $serializerCode = \TenantCloud\BetterReflection\Relocated\PHPStan\File\FileReader::read($serializerPath);
        $originalSerializerCode = $serializerCode;
        \unlink($serializerPath);
        $fileHelper = new \TenantCloud\BetterReflection\Relocated\PHPStan\File\FileHelper(__DIR__);
        $result = $this->runPhpstan(1);
        $this->assertSame(4, $result['totals']['file_errors'], \TenantCloud\BetterReflection\Relocated\Nette\Utils\Json::encode($result));
        $this->assertSame(0, $result['totals']['errors'], \TenantCloud\BetterReflection\Relocated\Nette\Utils\Json::encode($result));
    ...
    private function runPhpstan(int $expectedExitCode) : array
    {
        ...
        $output = \implode("\n", $outputLines);
        try {
            $json = \TenantCloud\BetterReflection\Relocated\Nette\Utils\Json::decode($output, \TenantCloud\BetterReflection\Relocated\Nette\Utils\Json::FORCE_ARRAY);
    ...
    private function relativizePath(string $path) : string
    {
        ...
        $helper = new \TenantCloud\BetterReflection\Relocated\PHPStan\File\SimpleRelativePathHelper(\str_replace('\\', '/', __DIR__ . '/PHP-Parser'));
