...
use TenantCloud\BetterReflection\Relocated\PHPStan\Rules\AlwaysFailRule;
...
class AnalyserTest extends \TenantCloud\BetterReflection\Relocated\PHPStan\Testing\TestCase
{
    public function testReturnErrorIfIgnoredMessagesDoesNotOccur() : void
    {
        $result = $this->runAnalyser(['#Unknown error#'], \true, __DIR__ . '/data/empty/empty.php', \false);
        $this->assertSame(['Ignored error pattern #Unknown error# was not matched in reported errors.'], $result);
    }
    ...
    {
        $result = $this->runAnalyser(['#Unknown error#'], \true, __DIR__ . '/data/empty/empty.php', \true);
        $this->assertEmpty($result);
    }
    public function testReportInvalidIgnorePatternEarly() : void
    {
        $result = $this->runAnalyser(['#Regexp syntax error'], \true, __DIR__ . '/data/parse-error.php', \false);
        $this->assertSame(["No ending delimiter '#' found in pattern: #Regexp syntax error"], $result);
    }
    public function testFileWithAnIgnoredError() : void
    {
        $result = $this->runAnalyser(['#Fail\\.#'], \true, __DIR__ . '/data/bootstrap-error.php', \false);
        $this->assertEmpty($result);
    }
    public function testIgnoringBrokenConfigurationDoesNotWork() : void
    {
        $this->markTestIncomplete();
        $result = $this->runAnalyser(['#was not found while trying to analyse it#'], \true, __DIR__ . '/../../notAutoloaded/Baz.php', \false);
        $this->assertCount(2, $result);
        \assert($result[0] instanceof \TenantCloud\BetterReflection\Relocated\PHPStan\Analyser\Error);
    ...
    {
        $ignoreErrors = [['message' => '#Fail\\.#', 'path' => __DIR__ . '/data/bootstrap-error.php']];
        $result = $this->runAnalyser($ignoreErrors, \true, __DIR__ . '/data/bootstrap-error.php', \false);
        $this->assertCount(0, $result);
    }
    public function testIgnoreErrorByPathAndCount() : void
    {
        $ignoreErrors = [['message' => '#Fail\\.#', 'count' => 3, 'path' => __DIR__ . '/data/two-fails.php']];
        $result = $this->runAnalyser($ignoreErrors, \true, __DIR__ . '/data/two-fails.php', \false);
        $this->assertCount(0, $result);
    }
    ...
    {
        $ignoreErrors = [['message' => '#Fail\\.#', 'count' => 1, 'path' => __DIR__ . '/data/two-fails.php']];
        $result = $this->runAnalyser($ignoreErrors, \true, __DIR__ . '/data/two-fails.php', $onlyFiles);
        $this->assertCount(3, $result);
        $this->assertInstanceOf(\TenantCloud\BetterReflection\Relocated\PHPStan\Analyser\Error::class, $result[0]);
        ...
        $this->assertInstanceOf(\TenantCloud\BetterReflection\Relocated\PHPStan\Analyser\Error::class, $result[1]);
        ...
        $this->assertInstanceOf(\TenantCloud\BetterReflection\Relocated\PHPStan\Analyser\Error::class, $result[2]);
    ...
    {
        $ignoreErrors = [['message' => '#Fail\\.#', 'count' => 4, 'path' => __DIR__ . '/data/two-fails.php']];
        $result = $this->runAnalyser($ignoreErrors, \true, __DIR__ . '/data/two-fails.php', $onlyFiles);
    ...
    {
        $ignoreErrors = [['message' => '#Some custom error\\.#', 'count' => 2, 'path' => __DIR__ . '/data/two-fails.php']];
        $result = $this->runAnalyser($ignoreErrors, \true, __DIR__ . '/data/two-fails.php', \false);
        ...
        $this->assertSamePaths(__DIR__ . '/data/two-fails.php', $result[2]->getFile());
        $this->assertInstanceOf(\TenantCloud\BetterReflection\Relocated\PHPStan\Analyser\Error::class, $result[3]);
    ...
    {
        $ignoreErrors = [['message' => '#Fail\\.#', 'paths' => [__DIR__ . '/data/bootstrap-error.php']]];
        $result = $this->runAnalyser($ignoreErrors, \true, __DIR__ . '/data/bootstrap-error.php', \false);
        $this->assertCount(0, $result);
    }
    public function testIgnoreErrorByPathsMultipleUnmatched() : void
    {
        $ignoreErrors = [['message' => '#Fail\\.#', 'paths' => [__DIR__ . '/data/bootstrap-error.php', __DIR__ . '/data/another-path.php', '/data/yet-another-path.php']]];
        $result = $this->runAnalyser($ignoreErrors, \true, __DIR__ . '/data/bootstrap-error.php', \false);
        $this->assertCount(1, $result);
    ...
    {
        $ignoreErrors = [['message' => '#Fail\\.#', 'paths' => [__DIR__ . '/data/bootstrap-error.php', __DIR__ . '/data/another-path.php']]];
        $result = $this->runAnalyser($ignoreErrors, \true, __DIR__ . '/data/bootstrap-error.php', \false);
        $this->assertCount(1, $result);
        $this->assertIsString($result[0]);
    ...
    {
        $ignoreErrors = [['message' => '#Fail\\.#', 'path' => __DIR__ . '/data/not-existent-path.php']];
        $result = $this->runAnalyser($ignoreErrors, \true, __DIR__ . '/data/empty/empty.php', \false);
        $this->assertCount(1, $result);
    ...
    {
        return [[__DIR__ . '/data/traits-ignore/Foo.php'], [__DIR__ . '/data/traits-ignore/FooTrait.php']];
    }
    /**
    ...
    {
        $ignoreErrors = [['message' => '#Fail\\.#', 'path' => $pathToIgnore]];
        $result = $this->runAnalyser($ignoreErrors, \true, [__DIR__ . '/data/traits-ignore/Foo.php', __DIR__ . '/data/traits-ignore/FooTrait.php'], \true);
        $this->assertCount(0, $result);
    }
    public function testIgnoredErrorMissingMessage() : void
    {
        $ignoreErrors = [['path' => __DIR__ . '/data/empty/empty.php']];
    ...
    {
        $ignoreErrors = [['message' => '#Fail\\.#', 'path' => __DIR__ . '/data/bootstrap-error.php'], ['message' => '#Fail\\.#', 'path' => __DIR__ . '/data/two-fails.php']];
        $result = $this->runAnalyser($ignoreErrors, \true, [__DIR__ . '/data/two-fails.php'], $onlyFiles);
        $this->assertCount(0, $result);
    }
    /**
    ...
    {
        $ignoreErrors = [['message' => '#Fail\\.#', 'path' => __DIR__ . '/data/bootstrap-error.php', 'count' => 2], ['message' => '#Fail\\.#', 'path' => __DIR__ . '/data/two-fails.php', 'count' => 3]];
        $result = $this->runAnalyser($ignoreErrors, \true, [__DIR__ . '/data/two-fails.php'], $onlyFiles);
        $this->assertCount(0, $result);
    }
    /**
    ...
    {
        $result = $this->runAnalyser([], $reportUnmatchedIgnoredErrors, [__DIR__ . '/data/ignore-next-line.php'], \true);
        $this->assertCount($reportUnmatchedIgnoredErrors ? 4 : 3, $result);
        foreach ([10, 30, 34] as $i => $line) {
            $this->assertArrayHasKey($i, $result);
            $this->assertInstanceOf(\TenantCloud\BetterReflection\Relocated\PHPStan\Analyser\Error::class, $result[$i]);
    ...
    {
        $result = $this->runAnalyser([], $reportUnmatchedIgnoredErrors, [__DIR__ . '/data/ignore-line.php'], \true);
        $this->assertCount($reportUnmatchedIgnoredErrors ? 4 : 3, $result);
     ...
     */
    private function runAnalyser(array $ignoreErrors, bool $reportUnmatchedIgnoredErrors, $filePaths, bool $onlyFiles) : array
    {
        $analyser = $this->createAnalyser($reportUnmatchedIgnoredErrors);
        if (\is_string($filePaths)) {
            $filePaths = [$filePaths];
        }
        $ignoredErrorHelper = new \TenantCloud\BetterReflection\Relocated\PHPStan\Analyser\IgnoredErrorHelper(self::getContainer()->getByType(\TenantCloud\BetterReflection\Relocated\PHPStan\Command\IgnoredRegexValidator::class), $this->getFileHelper(), $ignoreErrors, $reportUnmatchedIgnoredErrors);
    ...
    private function createAnalyser(bool $reportUnmatchedIgnoredErrors) : \TenantCloud\BetterReflection\Relocated\PHPStan\Analyser\Analyser
    {
        $registry = new \TenantCloud\BetterReflection\Relocated\PHPStan\Rules\Registry([new \TenantCloud\BetterReflection\Relocated\PHPStan\Rules\AlwaysFailRule()]);
        ...
        $nodeScopeResolver = new \TenantCloud\BetterReflection\Relocated\PHPStan\Analyser\NodeScopeResolver($broker, self::getReflectors()[0], $this->getClassReflectionExtensionRegistryProvider(), $this->getParser(), $fileTypeMapper, self::getContainer()->getByType(\TenantCloud\BetterReflection\Relocated\PHPStan\Php\PhpVersion::class), $phpDocInheritanceResolver, $fileHelper, $typeSpecifier, \false, \false, \true, [], []);
        $lexer = new \TenantCloud\BetterReflection\Relocated\PhpParser\Lexer(['usedAttributes' => ['comments', 'startLine', 'endLine', 'startTokenPos', 'endTokenPos']]);
        $fileAnalyser = new \TenantCloud\BetterReflection\Relocated\PHPStan\Analyser\FileAnalyser($this->createScopeFactory($broker, $typeSpecifier), $nodeScopeResolver, new \TenantCloud\BetterReflection\Relocated\PHPStan\Parser\RichParser(new \TenantCloud\BetterReflection\Relocated\PhpParser\Parser\Php7($lexer), new \TenantCloud\BetterReflection\Relocated\PhpParser\NodeVisitor\NameResolver(), new \TenantCloud\BetterReflection\Relocated\PhpParser\NodeVisitor\NodeConnectingVisitor(), new \TenantCloud\BetterReflection\Relocated\PHPStan\NodeVisitor\StatementOrderVisitor()), new \TenantCloud\BetterReflection\Relocated\PHPStan\Dependency\DependencyResolver($fileHelper, $broker, new \TenantCloud\BetterReflection\Relocated\PHPStan\Dependency\ExportedNodeResolver($fileTypeMapper, $printer)), $reportUnmatchedIgnoredErrors);
        return new \TenantCloud\BetterReflection\Relocated\PHPStan\Analyser\Analyser($fileAnalyser, $registry, $nodeScopeResolver, 50);
    }
}
