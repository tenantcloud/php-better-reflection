 ...
 * @group exec
 */
class CommandHelperTest extends \TenantCloud\BetterReflection\Relocated\PHPUnit\Framework\TestCase
{
    ...
    public function testBegin(string $input, string $expectedOutput, ?string $projectConfigFile, ?string $level, array $expectedParameters, bool $expectException) : void
    {
        ...
        try {
            $result = \TenantCloud\BetterReflection\Relocated\PHPStan\Command\CommandHelper::begin(new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Input\StringInput($input), $output, [__DIR__], null, null, null, [], $projectConfigFile, null, $level, \false, \true);
            if ($expectException) {
                $this->fail();
            }
    ...
    {
        $result = \TenantCloud\BetterReflection\Relocated\PHPStan\Command\CommandHelper::begin(new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Input\StringInput(''), new \TenantCloud\BetterReflection\Relocated\Symfony\Component\Console\Output\NullOutput(), [__DIR__], null, null, null, [], $configFile, null, '0', \false, \true);
        $parameters = $result->getContainer()->getParameters();
        foreach ($expectedParameters as $name => $expectedValue) {
            $this->assertArrayHasKey($name, $parameters);
            $this->assertSame($expectedValue, $parameters[$name]);
