 ...
 */
class OverridingMethodRuleTest extends \TenantCloud\BetterReflection\Relocated\PHPStan\Testing\RuleTestCase
{
    /** @var int */
    private $phpVersionId;
    protected function getRule() : \TenantCloud\BetterReflection\Relocated\PHPStan\Rules\Rule
    {
        return new \TenantCloud\BetterReflection\Relocated\PHPStan\Rules\Methods\OverridingMethodRule(new \TenantCloud\BetterReflection\Relocated\PHPStan\Php\PhpVersion($this->phpVersionId), new \TenantCloud\BetterReflection\Relocated\PHPStan\Rules\Methods\MethodSignatureRule(\true, \true), \false);
    }
    public function dataOverridingFinalMethod() : array
    {
        return [[70300, 'compatible', 'compatible'], [70400, 'contravariant', 'covariant']];
    }
    /**
     ...
     */
    public function testParameterContravariance(string $file, int $phpVersion, array $expectedErrors) : void
    {
        if (!self::$useStaticReflectionProvider) {
            $this->markTestSkipped('Test requires static reflection.');
        }
     ...
     * @param mixed[] $expectedErrors
     */
    public function testReturnTypeCovariance(int $phpVersion, array $expectedErrors) : void
    {
        if (!self::$useStaticReflectionProvider) {
            $this->markTestSkipped('Test requires static reflection.');
        }
    ...
    public function testParle(int $phpVersion, string $contravariantMessage, string $covariantMessage) : void
    {
        ...
        $this->phpVersionId = $phpVersion;
        $this->analyse([__DIR__ . '/data/parle.php'], [['Parameter #1 $state (int) of method OverridingParle\\Foo::pushState() is not ' . $contravariantMessage . ' with parameter #1 $state (string) of method Parle\\RLexer::pushState().', 8], ['Return type string of method OverridingParle\\Foo::pushState() is not ' . $covariantMessage . ' with return type int of method Parle\\RLexer::pushState().', 8]]);
    }
    ...
    {
        return [[70100, [['Parameter #1 $foo (mixed) of method ParameterTypeWidening\\Bar::doFoo() does not match parameter #1 $foo (string) of method ParameterTypeWidening\\Foo::doFoo().', 18]]], [70200, []]];
    }
    /**
