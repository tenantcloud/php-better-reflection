...
namespace TenantCloud\BetterReflection\Relocated\StrictComparison;
...
class Foo
{
    public function doFoo()
    {
        ...
        doFoo() === doBar();
        1 === null;
        new \TenantCloud\BetterReflection\Relocated\StrictComparison\Bar() === 1;
        // wrong
        /** @var Foo[]|Collection|bool $unionIterableType */
        $unionIterableType = doFoo();
        1 === $unionIterableType;
        \false === $unionIterableType;
        $unionIterableType === [new \TenantCloud\BetterReflection\Relocated\StrictComparison\Foo()];
        $unionIterableType === new \TenantCloud\BetterReflection\Relocated\StrictComparison\Collection();
        /** @var bool $boolean */
        $boolean = doFoo();
        \true === $boolean;
        \false === $boolean;
        ...
        switch ('foo') {
            case 'foo':
                $trueOrFalseInSwitch = \true;
                break;
        }
        if ($trueOrFalseInSwitch === \true) {
        }
        1.0 === 1;
        1 === 1.0;
...
class ComparingAgainstEmptyArray
{
     ...
     * @param mixed[] $mixeds
     */
    public function doFoo(array $strings, array $mixeds)
    {
        if ($strings === []) {
        }
        if ($mixeds === []) {
     ...
     * @param mixed[] $mixeds
     */
    public function doBar(array $strings, array $mixeds)
    {
        if ([] === $strings) {
        }
        if ([] === $mixeds) {
...
{
    private const MAPPING = [\stdClass::class, \TenantCloud\BetterReflection\Relocated\StrictComparison\RandomAbstractClass::class];
    public function test(int $key)
    {
        if (\is_subclass_of(self::MAPPING[$key], \TenantCloud\BetterReflection\Relocated\StrictComparison\RandomAbstractClass::class) === \false) {
            return;
        }
    }
}
