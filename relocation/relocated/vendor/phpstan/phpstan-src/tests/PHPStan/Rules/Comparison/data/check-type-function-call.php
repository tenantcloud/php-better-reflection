...
namespace TenantCloud\BetterReflection\Relocated\CheckTypeFunctionCall;
...
class Foo
{
     ...
     */
    public function doFoo(int $integer, $integerOrString, string $string, callable $callable, array $array, array $arrayOfInt)
    {
        if (\is_int($integer)) {
            // always true
        }
        if (\is_int($integerOrString)) {
            // fine
        }
        if (\is_int($string)) {
            // always false
        ...
        $className = 'Foo';
        if (\is_a($className, \Throwable::class, \true)) {
            // should be fine
        }
...
class IsSubclassOfTest
{
    public function doFoo(string $string, ?string $nullableString)
    {
        ...
        \is_subclass_of($nullableString, 'TenantCloud\\BetterReflection\\Relocated\\Foo');
    }
}
class DefinedConstant
{
    public function doFoo()
    {
        ...
        $foo = new \TenantCloud\BetterReflection\Relocated\CheckTypeFunctionCall\Foo();
...
class InArray
{
    public function doFoo(string $s, int $i, $mixed)
    {
        if (\in_array('foo', $mixed, \true)) {
        }
        if (\in_array($s, ['foo', 'bar'], \true)) {
        }
        if (\in_array($i, ['foo', 'bar'], \true)) {
        }
        $fooOrBar = 'foo';
        if (\rand(0, 1) === 0) {
            $fooOrBar = 'bar';
        }
        if (\in_array($fooOrBar, ['baz', 'lorem'], \true)) {
        }
        if (\in_array($fooOrBar, ['foo', 'bar'], \true)) {
        }
        if (\in_array('foo', ['foo'], \true)) {
        }
        if (\in_array('foo', ['foo', 'bar'], \true)) {
        }
        $arr = ['foo', 'bar'];
        if (\rand(0, 1) === 0) {
            $arr = \false;
        }
        if (\in_array('foo', $arr, \true)) {
        }
    }
    /**
     * @param string $s
     ...
     */
    public function doBar(string $s, array $strings)
    {
        if (\in_array($s, $strings, \true)) {
        }
    }
    /**
     * @param string $s
     ...
     */
    public function doBaz(string $s, array $mixedArray, array $stringsOrFloats)
    {
        if (\in_array($s, $mixedArray, \true)) {
        }
        if (\in_array('s', $mixedArray, \true)) {
        }
        if (\in_array($s, $stringsOrFloats, \true)) {
        }
        if (\in_array('s', $stringsOrFloats, \true)) {
        }
    }
    public function checkByCondition(int $x)
    {
        ...
        if (\in_array('foo', $data, \true)) {
        }
        if (\in_array('bar', $data, \true)) {
        }
    }
...
class PropertyExistsUniversalCrate
{
     ...
     */
    public function doFoo(\stdClass $std, $stdOrSelf)
    {
...
class MethodExists
{
    public function testWithStringFirstArgument() : void
    {
        /** @var string $string */
        $string = doFoo();
        if (\method_exists(\TenantCloud\BetterReflection\Relocated\CheckTypeFunctionCall\MethodExists::class, 'testWithStringFirstArgument')) {
        }
        if (\method_exists(\TenantCloud\BetterReflection\Relocated\CheckTypeFunctionCall\MethodExists::class, 'undefinedMethod')) {
        }
        if (\method_exists(\TenantCloud\BetterReflection\Relocated\CheckTypeFunctionCall\MethodExists::class, $string)) {
        }
    ...
    public function testWithNewObjectInFirstArgument() : void
    {
        /** @var string $string */
        $string = doFoo();
        if (\method_exists(new \TenantCloud\BetterReflection\Relocated\CheckTypeFunctionCall\MethodExists(), 'testWithNewObjectInFirstArgument')) {
        }
        if (\method_exists(new \TenantCloud\BetterReflection\Relocated\CheckTypeFunctionCall\MethodExists(), 'undefinedMethod')) {
        }
        if (\method_exists(new \TenantCloud\BetterReflection\Relocated\CheckTypeFunctionCall\MethodExists(), $string)) {
        }
    }
}
...
class CheckIsStringInElseIf
{
    ...
    {
        if ($a instanceof \TenantCloud\BetterReflection\Relocated\CheckTypeFunctionCall\Foo) {
            return \true;
        } elseif (!\is_string($a)) {
...
class AssertIsNumeric
{
    ...
    {
        $ratingMapping = ['UR' => 0, 'NR' => 0, 'G' => 0, 'PG' => 8, 'PG-13' => 13, 'R' => 15, 'NC-17' => 17];
...
class Bug2221
{
    ...
    {
        if (\property_exists(\TenantCloud\BetterReflection\Relocated\CheckTypeFunctionCall\Bug2221::class, 'foo')) {
        }
        \assert(\property_exists(\TenantCloud\BetterReflection\Relocated\CheckTypeFunctionCall\Bug2221::class, 'foo'));
        if (\property_exists(\TenantCloud\BetterReflection\Relocated\CheckTypeFunctionCall\Bug2221::class, 'bar')) {
        }
        \assert(\property_exists(\TenantCloud\BetterReflection\Relocated\CheckTypeFunctionCall\Bug2221::class, 'bar'));
    }
...
class InArray2
{
    ...
    public function doBar($std, $stdClassesOrNull) : void
    {
        ...
        if (\in_array($std, $stdClassesOrNull, \true)) {
        }
    }
}
