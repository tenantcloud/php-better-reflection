...
function () {
    ...
    \parse_str($parseStrParameter, $parseStrParameter, $parseStrParameter);
    $foo ?? $foo;
    // $foo undefined (once - after ??)
    $bar[0] ?? null;
    // OK
    $willBeUnset = 'foo';
    unset($willBeUnset);
    ...
    $serviceDefinition = $containerBuilder->addDefinition($serviceName = prefix('cache'))->setAutowired(\false);
    instantiate($serviceName);
    function () use(&$errorHandler) {
        $errorHandler->handle();
        // variable is fine here
    };
    $refObject =& refFunction();
    $refObject->foo;
    funcWithSpecialParameter(1, 2, $variableDefinedInsideTheFunction);
    echo $variableDefinedInsideTheFunction;
    $fooObject = new \TenantCloud\BetterReflection\Relocated\DefinedVariables\Foo();
    ...
    \TenantCloud\BetterReflection\Relocated\DefinedVariables\Foo::doStaticFoo(1, 2, $variableDefinedInStaticMethodPassedByReference);
    ...
    doFoo($variableAssignedInIsset);
    unset($unsettingUndefinedVariable);
    // it's fine from PHP POV
    ...
    echo $assignedInKey;
    if (($isInstanceOf = $fooObject) instanceof \TenantCloud\BetterReflection\Relocated\DefinedVariables\Foo && $isInstanceOf) {
    }
    echo $isInstanceOf;
    ...
    } catch (\TenantCloud\BetterReflection\Relocated\DefinedVariables\Exception $e) {
        if (doFoo()) {
            throw $e;
        } else {
            return;
    ...
    } catch (\TenantCloud\BetterReflection\Relocated\FooException $e) {
        $variableDefinedInTryAndAllCatches = 1;
        $variableAvailableInAllCatches = 1;
        $variableDefinedOnlyInOneCatch = 'foo';
        echo $variableDefinedInTry;
    } catch (\TenantCloud\BetterReflection\Relocated\BarException $e) {
        $variableDefinedInTryAndAllCatches = 1;
    ...
    switch (\true) {
        ...
        case $variableInSecondCase = \false:
            ...
            echo $variableInSecondCase;
            echo $variableAssignedInSecondCase = \true;
            break;
        case whatever():
            echo $variableInFirstCase;
            echo $variableInSecondCase;
            $variableInFallthroughCase = \true;
            echo $variableAssignedInSecondCase;
        // surely undefined
        case foo():
            echo $variableInFallthroughCase;
            // might be undefined
            echo $variableInFirstCase;
        default:
    }
    switch (\true) {
        default:
            $variableFromDefaultFirst = \true;
        case 1:
            echo $variableFromDefaultFirst;
    }
    ...
    [] ? \true : ($anotherMaybeDefinedInTernary = 'foo');
    ...
    switch ($definedInSwitchCond = \true) {
        default:
    ...
    if (!\TenantCloud\BetterReflection\Relocated\DefinedVariables\Foo::doStaticFoo(1, 2, $variableDefinedInBooleaNotByReference)) {
    }
