...
$stubs = ['../../resources/functionMap.php', '../../resources/functionMap_php74delta.php', '../../resources/functionMap_php80delta.php', '../../resources/functionMetadata.php', '../../vendor/hoa/consistency/Prelude.php', '../../vendor/composer/InstalledVersions.php'];
$stubFinder = \TenantCloud\BetterReflection\Relocated\Isolated\Symfony\Component\Finder\Finder::create();
...
return ['prefix' => null, 'finders' => [], 'files-whitelist' => $stubs, 'patchers' => [function (string $filePath, string $prefix, string $content) : string {
    ...
    return \str_replace('TenantCloud\\BetterReflection\\Relocated\\|Nette\\DI\\Statement', \sprintf('TenantCloud\\BetterReflection\\Relocated\\|\\%s\\Nette\\DI\\Statement', $prefix), $content);
}, function (string $filePath, string $prefix, string $content) : string {
    if ($filePath !== 'vendor/nette/di/src/DI/Config/DefinitionSchema.php') {
        return $content;
    }
    $content = \str_replace(\sprintf('TenantCloud\\BetterReflection\\Relocated\\\'%s\\callable', $prefix), '\'callable', $content);
    $content = \str_replace('TenantCloud\\BetterReflection\\Relocated\\|Nette\\DI\\Definitions\\Statement', \sprintf('TenantCloud\\BetterReflection\\Relocated\\|%s\\Nette\\DI\\Definitions\\Statement', $prefix), $content);
    return $content;
}, function (string $filePath, string $prefix, string $content) : string {
    if ($filePath !== 'vendor/nette/di/src/DI/Extensions/ExtensionsExtension.php') {
        return $content;
    }
    $content = \str_replace(\sprintf('TenantCloud\\BetterReflection\\Relocated\\\'%s\\string', $prefix), '\'string', $content);
    $content = \str_replace('TenantCloud\\BetterReflection\\Relocated\\|Nette\\DI\\Definitions\\Statement', \sprintf('TenantCloud\\BetterReflection\\Relocated\\|%s\\Nette\\DI\\Definitions\\Statement', $prefix), $content);
    return $content;
}, function (string $filePath, string $prefix, string $content) : string {
    ...
    return \str_replace(\sprintf('TenantCloud\\BetterReflection\\Relocated\\%s\\PHPUnit\\Framework\\TestCase', $prefix), 'TenantCloud\\BetterReflection\\Relocated\\PHPUnit\\Framework\\TestCase', $content);
}, function (string $filePath, string $prefix, string $content) : string {
    if ($filePath !== 'src/Testing/LevelsTestCase.php') {
        return $content;
    }
    return \str_replace([\sprintf('TenantCloud\\BetterReflection\\Relocated\\%s\\PHPUnit\\Framework\\AssertionFailedError', $prefix), \sprintf('TenantCloud\\BetterReflection\\Relocated\\%s\\PHPUnit\\Framework\\TestCase', $prefix)], ['\\PHPUnit\\Framework\\AssertionFailedError', '\\PHPUnit\\Framework\\TestCase'], $content);
}, function (string $filePath, string $prefix, string $content) : string {
    ...
    $neon = \TenantCloud\BetterReflection\Relocated\Nette\Neon\Neon::decode($content);
    $updatedNeon = $neon;
    ...
    return \TenantCloud\BetterReflection\Relocated\Nette\Neon\Neon::encode($updatedNeon, \TenantCloud\BetterReflection\Relocated\Nette\Neon\Neon::BLOCK);
}, function (string $filePath, string $prefix, string $content) : string {
    if (!\in_array($filePath, ['src/Testing/TestCaseSourceLocatorFactory.php', 'src/Testing/TestCase.php'], \true)) {
        return $content;
    }
    return \str_replace(\sprintf('TenantCloud\\BetterReflection\\Relocated\\%s\\Composer\\Autoload\\ClassLoader', $prefix), 'TenantCloud\\BetterReflection\\Relocated\\Composer\\Autoload\\ClassLoader', $content);
}, function (string $filePath, string $prefix, string $content) : string {
    ...
    return \str_replace(\sprintf('TenantCloud\\BetterReflection\\Relocated\\%s\\ReflectionUnionType', $prefix), 'ReflectionUnionType', $content);
}, function (string $filePath, string $prefix, string $content) : string {
    if (\strpos($filePath, 'src/') !== 0) {
        return $content;
    }
    return \str_replace(\sprintf('TenantCloud\\BetterReflection\\Relocated\\%s\\Attribute', $prefix), 'Attribute', $content);
}, function (string $filePath, string $prefix, string $content) : string {
