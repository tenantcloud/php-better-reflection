...
final class Decoder
{
    ...
    public function decode(string $input)
    {
        ...
        if ($this->tokens === \false) {
            throw new \TenantCloud\BetterReflection\Relocated\Nette\Neon\Exception('Invalid UTF-8 sequence.');
        }
        ...
        for (; $n < $count; $n++) {
            ...
            if ($t === ',') {
                ...
                if ($hasValue) {
                    ...
                    $n++;
                    if ($value instanceof \TenantCloud\BetterReflection\Relocated\Nette\Neon\Entity && $value->value === \TenantCloud\BetterReflection\Relocated\Nette\Neon\Neon::CHAIN) {
                        \end($value->attributes)->attributes = $this->parse(\false, []);
                    } else {
                        $value = new \TenantCloud\BetterReflection\Relocated\Nette\Neon\Entity($value, $this->parse(\false, []));
                    }
                } else {
                    $n++;
                ...
                if ($hasValue) {
                    if ($value instanceof \TenantCloud\BetterReflection\Relocated\Nette\Neon\Entity) {
                        // Entity chaining
                        if ($value->value !== \TenantCloud\BetterReflection\Relocated\Nette\Neon\Neon::CHAIN) {
                            $value = new \TenantCloud\BetterReflection\Relocated\Nette\Neon\Entity(\TenantCloud\BetterReflection\Relocated\Nette\Neon\Neon::CHAIN, [$value]);
                        }
                        $value->attributes[] = new \TenantCloud\BetterReflection\Relocated\Nette\Neon\Entity($converted);
                    } else {
                        $this->error();
                    }
                } else {
    ...
    private function error(string $message = "Unexpected '%s'")
    {
        ...
        throw new \TenantCloud\BetterReflection\Relocated\Nette\Neon\Exception(\str_replace('%s', $token, $message) . " on line {$line}, column {$col}.");
    }
}
