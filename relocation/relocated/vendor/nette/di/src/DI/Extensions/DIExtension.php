 ...
 * DI extension.
 */
final class DIExtension extends \TenantCloud\BetterReflection\Relocated\Nette\DI\CompilerExtension
{
    /** @var array */
    public $exportedTags = [];
    /** @var array */
    ...
    public function afterCompile(\TenantCloud\BetterReflection\Relocated\Nette\PhpGenerator\ClassType $class)
    {
        ...
        if ($this->debugMode && ($this->config->debugger ?? $this->getContainerBuilder()->getByType(\TenantCloud\BetterReflection\Relocated\Tracy\Bar::class))) {
            $this->enableTracyIntegration();
        }
        $this->initializeTaggedServices();
    }
    private function restrictTags(\TenantCloud\BetterReflection\Relocated\Nette\PhpGenerator\ClassType $class) : void
    {
    ...
    private function restrictTypes(\TenantCloud\BetterReflection\Relocated\Nette\PhpGenerator\ClassType $class) : void
    {
    ...
    {
        \TenantCloud\BetterReflection\Relocated\Nette\Bridges\DITracy\ContainerPanel::$compilationTime = $this->time;
        $this->initialization->addBody($this->getContainerBuilder()->formatPhp('?;', [new \TenantCloud\BetterReflection\Relocated\Nette\DI\Definitions\Statement('@Tracy\\Bar::addPanel', [new \TenantCloud\BetterReflection\Relocated\Nette\DI\Definitions\Statement(\TenantCloud\BetterReflection\Relocated\Nette\Bridges\DITracy\ContainerPanel::class)])]));
    }
}
