...
abstract class CompilerExtension
{
    ...
    /** @return static */
    public function setCompiler(\TenantCloud\BetterReflection\Relocated\Nette\DI\Compiler $compiler, string $name)
    {
        $this->initialization = new \TenantCloud\BetterReflection\Relocated\Nette\PhpGenerator\Closure();
    ...
    public function setConfig($config)
    {
        if (!\is_array($config) && !\is_object($config)) {
            throw new \TenantCloud\BetterReflection\Relocated\Nette\InvalidArgumentException();
        }
        $this->config = $config;
        return $this;
    }
    ...
    {
        return \is_object($this->config) ? \TenantCloud\BetterReflection\Relocated\Nette\Schema\Expect::from($this->config) : \TenantCloud\BetterReflection\Relocated\Nette\Schema\Expect::array();
    }
    /**
    ...
    public function validateConfig(array $expected, array $config = null, string $name = null) : array
    {
        ...
        if ($extra = \array_diff_key((array) $config, $expected)) {
            ...
            $hint = \TenantCloud\BetterReflection\Relocated\Nette\Utils\Helpers::getSuggestion(\array_keys($expected), \key($extra));
            $extra = $hint ? \key($extra) : \implode("', '{$name} › ", \array_keys($extra));
            throw new \TenantCloud\BetterReflection\Relocated\Nette\DI\InvalidConfigurationException("Unknown configuration option '{$name} › {$extra}'" . ($hint ? ", did you mean '{$name} › {$hint}'?" : '.'));
        }
        return \TenantCloud\BetterReflection\Relocated\Nette\Schema\Helpers::merge($config, $expected);
    }
    ...
    public function loadDefinitionsFromConfig(array $configList) : void
    {
        ...
        foreach ($configList as $key => $config) {
            ...
            $res[$key] = \TenantCloud\BetterReflection\Relocated\Nette\DI\Helpers::prefixServiceName($config, $this->name);
        }
    ...
    public function getInitialization() : \TenantCloud\BetterReflection\Relocated\Nette\PhpGenerator\Closure
    {
