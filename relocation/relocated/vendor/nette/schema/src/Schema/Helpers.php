...
final class Helpers
{
    ...
    {
        if ($types = \TenantCloud\BetterReflection\Relocated\Nette\Utils\Reflection::getPropertyTypes($prop)) {
            return \implode('|', $types);
        } elseif ($type = \preg_replace('#\\s.*#', '', (string) self::parseAnnotation($prop, 'var'))) {
            $class = \TenantCloud\BetterReflection\Relocated\Nette\Utils\Reflection::getPropertyDeclaringClass($prop);
            return \preg_replace_callback('#[\\w\\\\]+#', function ($m) use($class) {
                return \TenantCloud\BetterReflection\Relocated\Nette\Utils\Reflection::expandClassName($m[0], $class);
            }, $type);
        }
        return null;
    }
    ...
    public static function formatValue($value) : string
    {
        if (\is_object($value)) {
            ...
            return "'" . \TenantCloud\BetterReflection\Relocated\Nette\Utils\Strings::truncate($value, 15, '...') . "'";
