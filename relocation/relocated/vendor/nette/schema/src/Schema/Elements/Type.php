...
use TenantCloud\BetterReflection\Relocated\Nette\Schema\DynamicParameter;
...
final class Type implements \TenantCloud\BetterReflection\Relocated\Nette\Schema\Schema
{
    use Base;
    ...
    {
        $this->type = \TenantCloud\BetterReflection\Relocated\Nette\Schema\DynamicParameter::class . '|' . $this->type;
        return $this;
    }
    ...
    {
        $this->itemsValue = $valueType instanceof \TenantCloud\BetterReflection\Relocated\Nette\Schema\Schema ? $valueType : new self($valueType);
        $this->itemsKey = $keyType instanceof \TenantCloud\BetterReflection\Relocated\Nette\Schema\Schema || $keyType === null ? $keyType : new self($keyType);
        return $this;
    }
    ...
    public function complete($value, \TenantCloud\BetterReflection\Relocated\Nette\Schema\Context $context)
    {
        ...
        if ($value !== null && $this->pattern !== null && !\preg_match("\1^(?:{$this->pattern})\$\1Du", $value)) {
            $context->addError("The %label% %path% expects to match pattern '%pattern%', %value% given.", \TenantCloud\BetterReflection\Relocated\Nette\Schema\Message::PATTERN_MISMATCH, ['value' => $value, 'pattern' => $this->pattern]);
            return;
        }
        if ($value instanceof \TenantCloud\BetterReflection\Relocated\Nette\Schema\DynamicParameter) {
            $expected = $this->type . ($this->range === [null, null] ? '' : ':' . \implode('..', $this->range));
            $context->dynamics[] = [$value, \str_replace(\TenantCloud\BetterReflection\Relocated\Nette\Schema\DynamicParameter::class . '|', '', $expected)];
        }
        if ($this->itemsValue) {
        ...
        if ($merge) {
            $value = \TenantCloud\BetterReflection\Relocated\Nette\Schema\Helpers::merge($value, $this->default);
        }
