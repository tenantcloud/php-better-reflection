...
use TenantCloud\BetterReflection\Relocated\RingCentral\Psr7\StreamDecoratorTrait;
class Str extends \TenantCloud\BetterReflection\Relocated\RingCentral\Psr7\StreamDecoratorTrait implements \TenantCloud\BetterReflection\Relocated\Psr\Http\Message\StreamInterface
{
}
/**
 * @covers RingCentral\Psr7\StreamDecoratorTrait
 */
class StreamDecoratorTraitTest extends \TenantCloud\BetterReflection\Relocated\PHPUnit_Framework_TestCase
{
    private $a;
    private $b;
    private $c;
    public function setUp()
    {
        ...
        \fwrite($this->c, 'foo');
        \fseek($this->c, 0);
        $this->a = \TenantCloud\BetterReflection\Relocated\RingCentral\Psr7\stream_for($this->c);
        $this->b = new \TenantCloud\BetterReflection\Relocated\RingCentral\Tests\Psr7\Str($this->a);
    }
    public function testCatchesExceptionsWhenCastingToString()
    {
        $s = $this->getMockBuilder('TenantCloud\\BetterReflection\\Relocated\\Psr\\Http\\Message\\StreamInterface')->setMethods(array('read'))->getMockForAbstractClass();
        $s->expects($this->once())->method('read')->will($this->throwException(new \Exception('foo')));
        $msg = '';
        \set_error_handler(function ($errNo, $str) use(&$msg) {
            $msg = $str;
        });
        echo new \TenantCloud\BetterReflection\Relocated\RingCentral\Tests\Psr7\Str($s);
