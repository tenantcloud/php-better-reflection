 ...
 */
class Request extends \TenantCloud\BetterReflection\Relocated\RingCentral\Psr7\MessageTrait implements \TenantCloud\BetterReflection\Relocated\Psr\Http\Message\RequestInterface
{
    /** @var string */
    private $method;
    /** @var null|string */
    ...
    {
        if (\is_string($uri)) {
            $uri = new \TenantCloud\BetterReflection\Relocated\RingCentral\Psr7\Uri($uri);
        } elseif (!$uri instanceof \TenantCloud\BetterReflection\Relocated\Psr\Http\Message\UriInterface) {
            throw new \InvalidArgumentException('TenantCloud\\BetterReflection\\Relocated\\URI must be a string or Psr\\Http\\Message\\UriInterface');
        }
    ...
    public function withUri(\TenantCloud\BetterReflection\Relocated\Psr\Http\Message\UriInterface $uri, $preserveHost = \false)
    {
        if ($uri === $this->uri) {
            return $this;
        }
